...
var EList = {
	...
	{
		var listIndex = tr.closest('.kda-ie-tbl').attr('data-list-index');
		this.cfields[listIndex] = {};
		tr.find('.kda-ie-field-select').each(function(index){
			$(this).find('input').each(function(){
				...
				if(this.name.indexOf('SETTINGS[FIELDS_LIST][')==0)
				{
					EList.SetHiddenFieldVal(this, this.value);
				}
			});
...
var EImport = {
		...
		$.ajax({
			...
			error: function(data){
				if(data && data.responseText)
				{
					...
					/*else if(data.responseText.indexOf("Fatal error: Allowed memory size")!=-1)
					{
						obj.RunAutoConinue();
						return;
					}*/
	...
	RunAutoConinue: function()
	{
		...
		$.ajax({
			...
			error: function(){
				$('#kda_ie_auto_continue_time').html('');
				obj.TimeoutOnAutoConinue();
			}
		});
	},
...
var ESettings = {
	...
	BindConversionEvents: function()
	{
		...
		$('.kda-ie-settings-conversion:not([data-events-init]), .kda-ie-conv-condition-group-field:not([data-events-init])').each(function(){
			var parent = this;
			...
			$('span.kda-ie-conv-select-value', parent).bind('click', function(e){
				...
				selectObj.bind('change', function(){
					...
					if($('input[type="hidden"]', fieldObj).val()!=optval)
					{
			...
			$('select.field_cell, span.field_cell input[type="hidden"], select.field_when, span.field_when input[type="hidden"]', parent).bind('change', function(){
				...
				var cell = $('.field_cell input[type="hidden"]', parent).val();
				var when = $('.field_when input[type="hidden"]', parent).val();
				$('select.field_when, span.field_when', parent).show();
				$('.field_from', parent).show();
				if(cell=='GROUP')
				{
					obj.SetGroupConditions(parent);
				}
				else
				{
					obj.UnsetGroupConditions(parent);
	...
	SetGroupConditions: function(wrap)
	{		
		new ConvGroupCondition(wrap);
	},
	
	UnsetGroupConditions: function(wrap)
	{		
		$('>.kda-ie-conv-condition-group', wrap).remove();
	},
	...
	{
		this.UnsetGroupConditions(div);
		$('span.kda-ie-conv-select, span.kda-ie-conv-field', div).show();
...
function ConvGroupCondition(convWrap)
{
	this.Init = function(convWrap)
	{
		var obj = this;
		this.convWrap = convWrap;
		$('span.field_when, .field_from', convWrap).hide();
		this.groupWrap = $('<div class="kda-ie-conv-condition-group"><div class="kda-ie-conv-condition-group-footer"><a href="javascript:void(0)" class="kda-ie-conv-condition-group-add-field">'+BX.message("KDA_IE_CONV_NEW_CONDITION")+'</a></div></div>');
		$('>span.kda-ie-conv-condition', convWrap).after(this.groupWrap);
		$('>.kda-ie-conv-condition-group-footer > .kda-ie-conv-condition-group-add-field', this.groupWrap).bind('click', function(){
			obj.AddField();
		}).trigger('click');
	};
	...
	{
		var fieldWrap = $('<div class="kda-ie-conv-condition-group-field"></div>');
		fieldWrap.append($('>span.kda-ie-conv-condition', convWrap).clone());
		$('>.kda-ie-conv-condition-group-footer', this.groupWrap).before(fieldWrap);
		ESettings.AddDefaultConversionVals(fieldWrap, fieldWrap);
		ESettings.BindConversionEvents();
	}
