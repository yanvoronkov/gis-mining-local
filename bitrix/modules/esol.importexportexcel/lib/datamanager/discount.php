...
{	
	...
	public function SaveDiscount($ID, $IBLOCK_ID, $arFieldsProductDiscount, $name='', $isOffer = false)
	{
		...
		foreach($arSites as $siteId)
		{
			...
			if(1 || $onlyVal)
			{
				...
				$findProduct = false;
				$discountXmlId = '';
				$dbRes = \CCatalogDiscount::GetList(array('ID'=>'ASC'), array('PRODUCT_ID'=>$ID, '%XML_ID'=>addcslashes($arFieldsProductDiscount['XML_ID'], '|')), false, false, array('ID', 'XML_ID'));
				...
				if(!$findProduct)
				{
					...
					$isUpdate = false;
					$suffix = '';
					$dbRes = \CCatalogDiscount::GetList(array('XML_ID'=>'ASC'), array(($customXmlId ? 'XML_ID' : '%XML_ID')=>($customXmlId ? $arFieldsProductDiscount['XML_ID'] : addcslashes($arFieldsProductDiscount['XML_ID'], '|'))), false, false, array('ID', 'XML_ID', 'CONDITIONS'));
					while(!$isUpdate && ($arDiscount = $dbRes->Fetch()))
					{
						...
						$lastError = '';
						$arCond = \KdaIE\Utils::Unserialize($arDiscount['CONDITIONS']);
						$childrenKey = -1;
	...
	public function SaveSaleDiscount($ID, $IBLOCK_ID, $arFieldsProductDiscount, $name, $isOffer = false)
	{
		...
		foreach($arSites as $siteId)
		{
			...
			if(!$customXmlId && !array_key_exists('VALUE', $arFieldsProductDiscount)) continue;
			...
			while($arDiscount = $dbRes->Fetch())
			{
				$arCond = \KdaIE\Utils::Unserialize($arDiscount['ACTIONS']);
