...
class Price
{
		...
		foreach($arPrices as $gid=>$arFieldsPrice)
		{
			...
			foreach($arSubPrices as $arSubPrice)
			{
				...
				if(isset($arFieldsPrice['PRICE']))
				{
					...
					if($arFieldsPrice['PRICE']!=='-') $arFieldsPrice['PRICE'] = $this->GetFloatVal($arFieldsPrice['PRICE'], $this->GetPricePrecision());
				}
	...
	public function GetCurrencyVal($val)
	{
		...
		{
			if(((strpos($val, Loc::getMessage('KDA_IE_PRICE_CURRENCY_RUB'))!==false && $this->params['DEFAULT_CURRENCY']!='BYN') || strpos($val, '₽')!==false || strpos($val, 'RUR')!==false) && isset($this->arCurrencies['RUB'])) $val = 'RUB';
			elseif(strpos($val, '€')!==false && isset($this->arCurrencies['EUR'])) $val = 'EUR';
			elseif(strpos($val, '$')!==false && isset($this->arCurrencies['USD'])) $val = 'USD';
			else
			{
