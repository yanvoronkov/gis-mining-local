...
class PriceD7 extends Price
{
	...
	{
		if(!isset($this->pricePrecision))
		{
			$this->pricePrecision = 2;
			if(is_callable(array('\Bitrix\Catalog\PriceTable', 'getTableName')) && in_array('PRICE', $this->priceFields))
			{
				if($arr = \Bitrix\Main\Application::getConnection()->query('SHOW COLUMNS FROM `'. \Bitrix\Catalog\PriceTable::getTableName() .'` LIKE "PRICE"')->Fetch())
				{
					if(preg_match('/\(\s*\d+\s*,\s*(\d+)\s*\)/', $arr['Type'], $m))
					{
						$this->pricePrecision = $m[1];
					}
				}
			}
		}
