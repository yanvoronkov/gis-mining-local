...
class Product
{
	...
	public function SaveStore($ID, $IBLOCK_ID, $arStores, $arOldStores=false)
	{
		...
		foreach($arStores as $sid=>$arFieldsStore)
		{
			...
			{
				$dbRes = \CCatalogStoreProduct::GetList(array(), array('PRODUCT_ID'=>$ID, 'STORE_ID'=>$sid), false, false, array_merge(array('ID', 'PRODUCT_ID', 'STORE_ID'), (is_array($arFieldsStore) ? array_keys($arFieldsStore) : array())));
			...
			foreach($arStoreData as $arPrice)
			{
				...
				if(!empty($arFieldsStore))
				{
					...
					if($arPrice['STORE_ID']) $arFieldsStore['STORE_ID'] = $arPrice['STORE_ID'];
	...
	public function SetProductQuantity($ID, $IBLOCK_ID=0)
	{
		...
		if($calcPrice)
		{
			$arCalcParams = \KdaIE\Utils::Unserialize(\Bitrix\Main\Config\Option::get('iblock', 'KDA_IBLOCK'.$IBLOCK_ID.'_PRICEQNT_CONFORMITY'));
