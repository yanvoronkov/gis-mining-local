...
class IblockElementTable extends Entity\DataManager
{
    ...
    public function AddComp($arFields, $bWorkFlow=false, $bUpdateSearch=true, $bResizePictures=false, $cnt=0)
    {
		...
		try{
			...
			return $this->AddCompCatch($ex, $arFields, $bWorkFlow, $bUpdateSearch, $bResizePictures, $cnt);
		}catch(\Error $ex){
			return $this->AddCompCatch($ex, $arFields, $bWorkFlow, $bUpdateSearch, $bResizePictures, $cnt);
		}
		return $result;
	...
	public function AddCompCatch($ex, $arFields, $bWorkFlow, $bUpdateSearch, $bResizePictures, $cnt)
	{
    ...
    public function UpdateComp($ID, $arFields, $bWorkFlow=false, $bUpdateSearch=true, $bResizePictures=false, $bCheckDiskQuota=true, $cnt=0)
    {
		...
		try{
			...
			return $this->UpdateCompCatch($ex, $ID, $arFields, $bWorkFlow, $bUpdateSearch, $bResizePictures, $bCheckDiskQuota, $cnt);
		}catch(\Error $ex){
			return $this->UpdateCompCatch($ex, $ID, $arFields, $bWorkFlow, $bUpdateSearch, $bResizePictures, $bCheckDiskQuota, $cnt);
		}
		return $result;
	...
	public function UpdateCompCatch($ex, $ID, $arFields, $bWorkFlow, $bUpdateSearch, $bResizePictures, $bCheckDiskQuota, $cnt)
	{
	...
	public static function GetListComp($arFilter, $arKeys, $arOrder=array(), $limit=false, $offset=false)
	{
		if(empty($arOrder)) $arOrder = array('ID'=>'ASC');
		if(!isset($arFilter['CHECK_PERMISSIONS'])) $arFilter['CHECK_PERMISSIONS'] = 'N';
		
		$arNavParams = false;
		if(is_array($limit) && isset($limit['nPageSize']))
		{
			$arNavParams = $limit;
			if(isset($limit['iNumPage'])) $offset = ((int)$limit['iNumPage']-1)*(int)$limit['nPageSize'];
			$limit = (int)$limit['nPageSize'];
		}
		elseif(is_array($limit) && isset($limit['nTopCount']))
		{
			$arNavParams = $limit;
			$limit = (int)$limit['nTopCount'];
			$offset = false;
		}
		else
		{
			...
			{
				$arNavParams = array();
				if($limit!=false) $arNavParams['nTopCount'] = $limit;
				if($offset!=false) $arNavParams['iNumPage'] = floor($offset/$limit);
			}
		}
		...
		if(!isset(self::$elemListHash[$hash]))
		{
			...
			if(class_exists('\Bitrix\Iblock\ElementTable'))
			{
				...
				if($arIblock['WORKFLOW']!='Y')
				{
					...
					elseif(\Bitrix\KdaImportexcel\DataManager\ElementPropertyTable::issetValueIndex() && count(preg_grep('/^(IBLOCK_ID|CHECK_PERMISSIONS|[=%]PROPERTY_\d+|[=%]PROPERTY_\d+_VALUE)$/', $arFilterKeys))==count($arFilterKeys) && is_array($arIblock) && $limit===false)
					{
		...
		if($mtype=='d7')
		{
			...
			if($offset!==false) $arParams['offset'] = $offset;
			$dbResult = \Bitrix\Iblock\ElementTable::getList($arParams);
			self::AddNavParamsComp($dbResult, $arFilter, $limit, $offset);
		}
		elseif(in_array($mtype, array('d7_props', 'props')))
		{
			...
			if(!empty($arNewFilter))
			{
				...
				{
					$dbResult = \CIblockElement::GetList($arOrder, $arFilter, false, $arNavParams, $arKeys);
				}
			}
		}
	...
	public static function FetchComp($dbRes)
	{
		if(is_callable(array($dbRes, 'GetNext'))) return $dbRes->GetNext();
		return $dbRes->Fetch();
	}
	
	public static function AddNavParamsComp(&$dbRes, $arFilter, $limit, $offset)
	{
		if($limit===false || $offset===false) return;
		if(!isset($dbRes->NavRecordCount)) $dbRes->NavRecordCount = \Bitrix\Iblock\ElementTable::getCount($arFilter);
		if(!isset($dbRes->NavPageCount)) $dbRes->NavPageCount = ceil($dbRes->NavRecordCount / $limit);
	}
