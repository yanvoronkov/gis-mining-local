...
$pdfRendererClassFile = KDAPHPExcel_Settings::getPdfRendererPath() . '/tcpdf.php';
if (file_exists($pdfRendererClassFile)) {
    $k_path_url = KDAPHPExcel_Settings::getPdfRendererPath();
...
/**
 *  KDAPHPExcel_Writer_PDF_tcPDF
 *
 *  @category    KDAPHPExcel
 *  @package     KDAPHPExcel_Writer_PDF
 *  @copyright   Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Writer_PDF_tcPDF extends KDAPHPExcel_Writer_PDF_Core implements KDAPHPExcel_Writer_IWriter
{
    /**
    ...
    public function save($pFilename = NULL)
    {
        ...
        if (!is_null($this->getOrientation())) {
            $orientation = ($this->getOrientation() == KDAPHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE)
                ? 'L'
                : 'P';
        }
        //  Override Paper Size
		...
		$pdf->Ln();
		$pdf->addTOC(1, $this->getFont(), '.', 'INDEX', 'B', array(128,0,0));
		$pdf->endTOCPage();*/
