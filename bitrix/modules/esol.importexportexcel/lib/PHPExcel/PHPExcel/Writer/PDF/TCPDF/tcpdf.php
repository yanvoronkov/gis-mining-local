	...
	public function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if ($newimage) {
			...
			if (($info === false) AND function_exists($gdfunction)) {
				try {
					// GD library
					$img = call_user_func($gdfunction, $file);
					if ($img !== false) {
						if ($resize) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						if (isset($dom[$key]['style']['vertical-align'])) {
							$dom[$key]['valign'] = strtoupper($dom[$key]['style']['vertical-align'][0]);
						}
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		$key = 0;
		while ($key < $maxel) {
			if(isset($dom[$key]['attribute']['data-bookmark']) && strlen($dom[$key]['attribute']['data-bookmark']) > 0)
			{
				$this->Bookmark($dom[$key]['attribute']['data-bookmark']);
			}
			...
			if ($dom[$key]['tag'] OR ($key == 0)) {
				...
				if (isset($dom[$key]['valign'])) {
					$valign = $dom[$key]['valign'];
				}
			...
			if ($dom[$key]['tag']) {
				if ($dom[$key]['opening']) {
					...
					if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
						...
						$this->MultiCell($cellw, $cellh, $cell_content, false, $lalign, false, 2, '', '', true, 0, true, true, 0, $valign, false);
						// restore some values
