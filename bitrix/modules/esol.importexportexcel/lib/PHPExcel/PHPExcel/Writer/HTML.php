 ...
 * @package	KDAPHPExcel_Writer_HTML
...
/**
 * KDAPHPExcel_Writer_HTML
 *
 * @category   KDAPHPExcel
 * @package	KDAPHPExcel_Writer_HTML
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Writer_HTML extends KDAPHPExcel_Writer_Abstract implements KDAPHPExcel_Writer_IWriter {
	/**
	 * KDAPHPExcel object
	 *
	 * @var KDAPHPExcel
	...
	/**
	 * Create a new KDAPHPExcel_Writer_HTML
	 *
	...
	private function _mapVAlign($vAlign) {
		switch ($vAlign) {
			case KDAPHPExcel_Style_Alignment::VERTICAL_BOTTOM:		return 'bottom';
			case KDAPHPExcel_Style_Alignment::VERTICAL_TOP:		return 'top';
			case KDAPHPExcel_Style_Alignment::VERTICAL_CENTER:
			case KDAPHPExcel_Style_Alignment::VERTICAL_JUSTIFY:	return 'middle';
	...
	private function _mapHAlign($hAlign) {
		switch ($hAlign) {
			case KDAPHPExcel_Style_Alignment::HORIZONTAL_GENERAL:				return false;
			case KDAPHPExcel_Style_Alignment::HORIZONTAL_LEFT:					return 'left';
			case KDAPHPExcel_Style_Alignment::HORIZONTAL_RIGHT:				return 'right';
			case KDAPHPExcel_Style_Alignment::HORIZONTAL_CENTER:
			case KDAPHPExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS:	return 'center';
			case KDAPHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY:				return 'justify';
	...
	private function _mapBorderStyle($borderStyle) {
		switch ($borderStyle) {
			case KDAPHPExcel_Style_Border::BORDER_NONE:				return 'none';
			case KDAPHPExcel_Style_Border::BORDER_DASHDOT:				return '1px dashed';
			case KDAPHPExcel_Style_Border::BORDER_DASHDOTDOT:			return '1px dotted';
			case KDAPHPExcel_Style_Border::BORDER_DASHED:				return '1px dashed';
			case KDAPHPExcel_Style_Border::BORDER_DOTTED:				return '1px dotted';
			case KDAPHPExcel_Style_Border::BORDER_DOUBLE:				return '3px double';
			case KDAPHPExcel_Style_Border::BORDER_HAIR:				return '1px solid';
			case KDAPHPExcel_Style_Border::BORDER_MEDIUM:				return '2px solid';
			case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOT:		return '2px dashed';
			case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT:	return '2px dotted';
			case KDAPHPExcel_Style_Border::BORDER_MEDIUMDASHED:		return '2px dashed';
			case KDAPHPExcel_Style_Border::BORDER_SLANTDASHDOT:		return '2px dashed';
			case KDAPHPExcel_Style_Border::BORDER_THICK:				return '3px solid';
			case KDAPHPExcel_Style_Border::BORDER_THIN:				return '1px solid';
	 ...
	 * @return KDAPHPExcel_Writer_HTML
	 */
	...
	public function generateHTMLHeader($pIncludeStyles = false) {
		// KDAPHPExcel object known?
		if (is_null($this->_phpExcel)) {
			throw new KDAPHPExcel_Writer_Exception('Internal KDAPHPExcel object not set to an instance of an object.');
		}
		...
		$html .= '<!-- Generated by KDAPHPExcel - http://www.phpexcel.net -->' . PHP_EOL;
		$html .= '<html>' . PHP_EOL;
		$html .= '  <head>' . PHP_EOL;
	...
	public function generateSheetData() {
		...
		foreach ($sheets as $sheet) {
			// Write table header
			$html .= $this->_generateTableHeader($sheet, (bool)(count($sheets) > 1));

			// Get worksheet dimension
			$dimension = explode(':', $sheet->calculateWorksheetDimension());
			$dimension[0] = KDAPHPExcel_Cell::coordinateFromString($dimension[0]);
			$dimension[0][0] = KDAPHPExcel_Cell::columnIndexFromString($dimension[0][0]) - 1;
			$dimension[1] = KDAPHPExcel_Cell::coordinateFromString($dimension[1]);
			$dimension[1][0] = KDAPHPExcel_Cell::columnIndexFromString($dimension[1][0]) - 1;
	...
	private function _extendRowsForChartsAndImages(KDAPHPExcel_Worksheet $pSheet, $row) {
		$rowMax = $row;
		$colMax = 'A';
		if ($this->_includeCharts) {
			foreach ($pSheet->getChartCollection() as $chart) {
				if ($chart instanceof KDAPHPExcel_Chart) {
				    $chartCoordinates = $chart->getTopLeftPosition();
				    $chartTL = KDAPHPExcel_Cell::coordinateFromString($chartCoordinates['cell']);
					$chartCol = KDAPHPExcel_Cell::columnIndexFromString($chartTL[0]);
					if ($chartTL[1] > $rowMax) {
						$rowMax = $chartTL[1];
						if ($chartCol > KDAPHPExcel_Cell::columnIndexFromString($colMax)) {
							$colMax = $chartTL[0];
						}
					}
				}
		...
		foreach ($pSheet->getDrawingCollection() as $drawing) {
			if ($drawing instanceof KDAPHPExcel_Worksheet_Drawing) {
			    $imageTL = KDAPHPExcel_Cell::coordinateFromString($drawing->getCoordinates());
				$imageCol = KDAPHPExcel_Cell::columnIndexFromString($imageTL[0]);
				if ($imageTL[1] > $rowMax) {
					$rowMax = $imageTL[1];
					if ($imageCol > KDAPHPExcel_Cell::columnIndexFromString($colMax)) {
						$colMax = $imageTL[0];
					}
				}
			}
	 ...
	 *
	 * @param	KDAPHPExcel_Worksheet	$pSheet			KDAPHPExcel_Worksheet
	 * @param	string				$coordinates	Cell coordinates
	 * @return	string
	 * @throws	KDAPHPExcel_Writer_Exception
	 */
	private function _writeImageInCell(KDAPHPExcel_Worksheet $pSheet, $coordinates) {
		// Construct HTML
		$html = '';
	 ...
	 * @throws	KDAPHPExcel_Writer_Exception
	 */
	private function _writeChartInCell(KDAPHPExcel_Worksheet $pSheet, $coordinates) {
		// Construct HTML
		$html = '';
		...
		foreach ($pSheet->getChartCollection() as $chart) {
			if ($chart instanceof KDAPHPExcel_Chart) {
				...
				if ($chartCoordinates['cell'] == $coordinates) {
					$chartFileName = KDAPHPExcel_Shared_File::sys_get_temp_dir().'/'.uniqid().'.png';
	...
	public function buildCSS($generateSurroundingHTML = true) {
		...
		foreach ($sheets as $sheet) {
			...
			$highestColumnIndex = KDAPHPExcel_Cell::columnIndexFromString($sheet->getHighestColumn()) - 1;
			$column = -1;
			...
			foreach ($sheet->getColumnDimensions() as $columnDimension) {
				if (($width = KDAPHPExcel_Shared_Drawing::cellDimensionToPixels($columnDimension->getWidth(), $this->_defaultFont)) >= 0) {
					$width = KDAPHPExcel_Shared_Drawing::pixelsToPoints($width);
					$column = KDAPHPExcel_Cell::columnIndexFromString($columnDimension->getColumnIndex()) - 1;
			...
			if ($rowDimension->getRowHeight() == -1) {
				$pt_height = KDAPHPExcel_Shared_Font::getDefaultRowHeightByFont($this->_phpExcel->getDefaultStyle()->getFont());
			} else {
	 ...
	 * Create CSS style
	 *
	 * @param	KDAPHPExcel_Style		$pStyle			KDAPHPExcel_Style
	 * @return	array
	 */
	private function _createCSSStyle(KDAPHPExcel_Style $pStyle) {
		// Construct CSS
		$css = '';
	...
	/**
	 * Create CSS style (KDAPHPExcel_Style_Alignment)
	 *
	 * @param	KDAPHPExcel_Style_Alignment		$pStyle			KDAPHPExcel_Style_Alignment
	 * @return	array
	 */
	private function _createCSSStyleAlignment(KDAPHPExcel_Style_Alignment $pStyle) {
		// Construct CSS
		$css = array();
	...
	/**
	 * Create CSS style (KDAPHPExcel_Style_Font)
	 *
	 * @param	KDAPHPExcel_Style_Font		$pStyle			KDAPHPExcel_Style_Font
	 * @return	array
	 */
	private function _createCSSStyleFont(KDAPHPExcel_Style_Font $pStyle) {
		// Construct CSS
		$css = array();
		...
		if ($pStyle->getUnderline() != KDAPHPExcel_Style_Font::UNDERLINE_NONE && $pStyle->getStrikethrough()) {
			$css['text-decoration'] = 'underline line-through';
		} else if ($pStyle->getUnderline() != KDAPHPExcel_Style_Font::UNDERLINE_NONE) {
	...
	/**
	 * Create CSS style (KDAPHPExcel_Style_Borders)
	 *
	 * @param	KDAPHPExcel_Style_Borders		$pStyle			KDAPHPExcel_Style_Borders
	 * @return	array
	 */
	private function _createCSSStyleBorders(KDAPHPExcel_Style_Borders $pStyle) {
		// Construct CSS
		$css = array();
	...
	/**
	 * Create CSS style (KDAPHPExcel_Style_Border)
	 *
	 * @param	KDAPHPExcel_Style_Border		$pStyle			KDAPHPExcel_Style_Border
	 * @return	string
	 */
	private function _createCSSStyleBorder(KDAPHPExcel_Style_Border $pStyle) {
		// Create CSS
//		$css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
	...
	/**
	 * Create CSS style (KDAPHPExcel_Style_Fill)
	 *
	 * @param	KDAPHPExcel_Style_Fill		$pStyle			KDAPHPExcel_Style_Fill
	 * @return	array
	 */
	private function _createCSSStyleFill(KDAPHPExcel_Style_Fill $pStyle) {
		// Construct HTML
		$css = array();

		// Create CSS
		$value = $pStyle->getFillType() == KDAPHPExcel_Style_Fill::FILL_NONE ?
	 ...
	 *
	 * @param	KDAPHPExcel_Worksheet	$pSheet		The worksheet for the table we are writing
	 * @return	string
	 * @throws	KDAPHPExcel_Writer_Exception
	 */
	private function _generateTableHeader($pSheet, $addBookmark=false) {
		...
		$bookmark = '';
		if($addBookmark) $bookmark = 'data-bookmark="'.htmlspecialchars($pSheet->getTitle()).'"';
		
		if (!$this->_useInlineCss) {
			$gridlines = $pSheet->getShowGridLines() ? ' gridlines' : '';
			$html .= '	<table border="0" cellpadding="0" cellspacing="0" id="sheet' . $sheetIndex . '" class="sheet' . $sheetIndex . $gridlines . '" '.$bookmark.'>' . PHP_EOL;
		} else {
			$style = isset($this->_cssStyles['table']) ?
				$this->_assembleCSS($this->_cssStyles['table']) : '';

			if ($this->_isPdf && $pSheet->getShowGridLines()) {
				$html .= '	<table border="1" cellpadding="1" id="sheet' . $sheetIndex . '" cellspacing="1" style="' . $style . '" '.$bookmark.'>' . PHP_EOL;
			} else {
				$html .= '	<table border="0" cellpadding="1" id="sheet' . $sheetIndex . '" cellspacing="0" style="' . $style . '" '.$bookmark.'>' . PHP_EOL;
			}
		}

		// Write <col> elements
		$highestColumnIndex = KDAPHPExcel_Cell::columnIndexFromString($pSheet->getHighestColumn()) - 1;
		$i = -1;
	 ...
	 * @throws	KDAPHPExcel_Writer_Exception
	 */
	private function _generateRow(KDAPHPExcel_Worksheet $pSheet, $pValues = null, $pRow = 0) {
		if (is_array($pValues)) {
			// Construct HTML
			$html = '';
			...
			foreach ($pValues as $cell) {
				$coordinate = KDAPHPExcel_Cell::stringFromColumnIndex($colNum) . ($pRow + 1);
				...
				// KDAPHPExcel_Cell
				if ($cell instanceof KDAPHPExcel_Cell) {
					$cellData = '';
					...
					if ($cell->getValue() instanceof KDAPHPExcel_RichText) {
						...
						if ($this->_preCalculateFormulas) {
							$cellData = KDAPHPExcel_Style_NumberFormat::toFormattedString(
						...
						if(!preg_match('/<a[^>]+class="kda\-ee\-conversion\-link"[^>]+href="([^"]*)"[^>]*>(.*)<\/a>/Uis', $cellData, $m))
						{
					...
					if (!$this->_useInlineCss) {
						...
						if ($sharedStyle->getAlignment()->getHorizontal() == KDAPHPExcel_Style_Alignment::HORIZONTAL_GENERAL
				...
				if (isset($this->_isBaseCell[$pSheet->getParent()->getIndex($pSheet)][$pRow + 1][$colNum])) {
					...
					$endCellCoord = KDAPHPExcel_Cell::stringFromColumnIndex($colNum + $colSpan - 1) . ($pRow + $rowSpan);
				...
				if ($writeCell) {
					...
					$html .= '			<td';
						if (!$this->_useInlineCss) {
							...
							//** Necessary redundant code for the sake of KDAPHPExcel_Writer_PDF **
					...
					// Image?
					$img = $this->_writeImageInCell($pSheet, $coordinate);
					if(preg_match('/^(.*)(<img[^>]*>)(.*)$/is', $img, $m) && preg_match('/^\s*(<a[^>]*>).*(<\/a>)\s*$/is', $cellData, $m2))
					{
						$img = $m[1].$m2[1].$m[2].$m2[2].$m[3];
						$cellData = '';
					}
					$html .= $img;
	...
	private function _calculateSpans()
	{
		...
		foreach ($sheetIndexes as $sheetIndex) {
			...
			foreach ($sheet->getMergeCells() as $cells) {
				list($cells, ) = KDAPHPExcel_Cell::splitRange($cells);
				$first = $cells[0];
				$last  = $cells[1];

				list($fc, $fr) = KDAPHPExcel_Cell::coordinateFromString($first);
				$fc = KDAPHPExcel_Cell::columnIndexFromString($fc) - 1;

				list($lc, $lr) = KDAPHPExcel_Cell::coordinateFromString($last);
				$lc = KDAPHPExcel_Cell::columnIndexFromString($lc) - 1;
			...
			$countColumns = KDAPHPExcel_Cell::columnIndexFromString($sheet->getHighestColumn());
	...
	private function _setMargins(KDAPHPExcel_Worksheet $pSheet) {
		$htmlPage = '@page { ';
		$htmlBody = 'body { ';

		$left = KDAPHPExcel_Shared_String::FormatNumber($pSheet->getPageMargins()->getLeft()) . 'in; ';
		$htmlPage .= 'left-margin: ' . $left;
		$htmlBody .= 'left-margin: ' . $left;
		$right = KDAPHPExcel_Shared_String::FormatNumber($pSheet->getPageMargins()->getRight()) . 'in; ';
		$htmlPage .= 'right-margin: ' . $right;
		$htmlBody .= 'right-margin: ' . $right;
		$top = KDAPHPExcel_Shared_String::FormatNumber($pSheet->getPageMargins()->getTop()) . 'in; ';
		$htmlPage .= 'top-margin: ' . $top;
		$htmlBody .= 'top-margin: ' . $top;
		$bottom = KDAPHPExcel_Shared_String::FormatNumber($pSheet->getPageMargins()->getBottom()) . 'in; ';
