...
class KDAPHPExcel_Style_NumberFormat extends KDAPHPExcel_Style_Supervisor implements KDAPHPExcel_IComparable
{
	...
	{
		if ($format === self::FORMAT_PERCENTAGE || $format === self::FORMAT_PERCENTAGE2) {
			...
			if(strpos($format, ',')) $format = str_replace(',', '.', $format);
	...
	private static function _complexNumberFormatMask($number, $mask) {
		if (($pos1 = strpos($mask,'E+')) !== false && ($pos2 = strpos($number,'E+')) !== false) {
			return self::toFormattedString(substr($number, 0, $pos2), substr($mask, 0, $pos1)).substr($number, $pos2);
		}
		...
		if (preg_match('/^(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy]/i', $format)) { // datetime format
			...
			if ($format === self::FORMAT_CURRENCY_EUR_SIMPLE) {
				...
				if (preg_match('/#?.*\?\/\?/', $format, $m)) {
					...
					$format = preg_replace('/([\?0])\x80$/', '$1', $format);
