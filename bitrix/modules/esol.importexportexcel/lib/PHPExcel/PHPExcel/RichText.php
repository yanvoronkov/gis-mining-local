...
class KDAPHPExcel_RichText implements KDAPHPExcel_IComparable
{
    ...
    public function getHtmlText()
    {
        ...
        foreach ($this->_richTextElements as $text) {
			...
			if(is_object($font))
			{
				if($font->getColor()->getRGB() && $font->getColor()->getRGB()!='000000') $style .= 'color:#'.$font->getColor()->getRGB().';';
				if($font->getBold()) $style .= 'font-weight:bold;';
				if($font->getItalic()) $style .= 'font-style:italic;';
				if($font->getUnderline()=='single') $style .= 'text-decoration:underline;';
				if($font->getSize() > 14) $style .= 'font-size:'.$font->getSize().'px;';
			}
			$returnValue .= (strlen($style) > 0) ? '<span style="'.$style.'">'.$text->getText().'</span>' : $text->getText();
