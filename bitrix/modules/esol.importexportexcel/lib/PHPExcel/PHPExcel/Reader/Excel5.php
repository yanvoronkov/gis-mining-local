...
class KDAPHPExcel_Reader_Excel5 extends KDAPHPExcel_Reader_Abstract implements KDAPHPExcel_Reader_IReader
{
	...
	public function load($pFilename)
	{
		try
		{
			return $this->loadDirect($pFilename);
		}
		catch(\Exception $ex)
		{
			if($ex->getMessage()=='Cannot read encrypted file')
			{
				$reader = \KDAPHPExcel_IOFactory::createReader('Excel5n');
				$reader->setLoadSheetsOnly($this->getLoadSheetsOnly());
				$reader->setReadDataOnly($this->getReadDataOnly());
				$reader->setReadFilter($this->getReadFilter());
				return $reader->load($pFilename);
			}
			throw new KDAPHPExcel_Reader_Exception($ex->getMessage());
		}
	}
	 ...
	 * @throws 	KDAPHPExcel_Reader_Exception
	 */
	public function loadDirect($pFilename)
	{
		// Read the OLE file
		$this->_loadOLE($pFilename);
