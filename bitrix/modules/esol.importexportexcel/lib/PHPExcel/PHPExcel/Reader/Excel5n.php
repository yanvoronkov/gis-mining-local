...
/**
 * KDAPHPExcel_Reader_Excel5n
 *
 * Copyright (c) 2006 - 2015 KDAPHPExcel
 *
 ...
 * @package    KDAPHPExcel_Reader_Excel5n
 * @copyright  Copyright (c) 2006 - 2015 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
...
class KDAPHPExcel_Reader_Excel5n extends KDAPHPExcel_Reader_Abstract implements KDAPHPExcel_Reader_IReader
{
    // ParseXL definitions
     ...
     *
     * @var KDAPHPExcel_Reader_Excel5n_RC4
     */
    private $rc4Key = null;
    ...
    {
        $this->readFilter = new KDAPHPExcel_Reader_DefaultReadFilter();
    }
    ...
    public function load($pFilename)
    {
        ...
        if (!$this->readDataOnly) {
            foreach ($this->objFonts as $objFont) {
                if (isset($objFont->colorIndex)) {
                    $color = KDAPHPExcel_Reader_Excel5n_Color::map($objFont->colorIndex, $this->palette, $this->version);
            ...
            foreach ($this->phpExcel->getCellXfCollection() as $objStyle) {
                ...
                if (isset($fill->startcolorIndex)) {
                    $startColor = KDAPHPExcel_Reader_Excel5n_Color::map($fill->startcolorIndex, $this->palette, $this->version);
                    $fill->getStartColor()->setRGB($startColor['rgb']);
                }
                if (isset($fill->endcolorIndex)) {
                    $endColor = KDAPHPExcel_Reader_Excel5n_Color::map($fill->endcolorIndex, $this->palette, $this->version);
                ...
                if (isset($top->colorIndex)) {
                    $borderTopColor = KDAPHPExcel_Reader_Excel5n_Color::map($top->colorIndex, $this->palette, $this->version);
                    $top->getColor()->setRGB($borderTopColor['rgb']);
                }
                if (isset($right->colorIndex)) {
                    $borderRightColor = KDAPHPExcel_Reader_Excel5n_Color::map($right->colorIndex, $this->palette, $this->version);
                    $right->getColor()->setRGB($borderRightColor['rgb']);
                }
                if (isset($bottom->colorIndex)) {
                    $borderBottomColor = KDAPHPExcel_Reader_Excel5n_Color::map($bottom->colorIndex, $this->palette, $this->version);
                    $bottom->getColor()->setRGB($borderBottomColor['rgb']);
                }
                if (isset($left->colorIndex)) {
                    $borderLeftColor = KDAPHPExcel_Reader_Excel5n_Color::map($left->colorIndex, $this->palette, $this->version);
                    $left->getColor()->setRGB($borderLeftColor['rgb']);
                }
                if (isset($diagonal->colorIndex)) {
                    $borderDiagonalColor = KDAPHPExcel_Reader_Excel5n_Color::map($diagonal->colorIndex, $this->palette, $this->version);
        ...
        if (!$this->readDataOnly && $this->drawingGroupData) {
            ...
            $reader = new KDAPHPExcel_Reader_Excel5n_Escher($escherWorkbook);
        ...
        foreach ($this->sheets as $sheet) {
            ...
            if (!$this->readDataOnly && $this->drawingData) {
                ...
                $reader = new KDAPHPExcel_Reader_Excel5n_Escher($escherWorksheet);
...
//                echo '<br />';
                ...
                if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
                    ...
                    $width = KDAPHPExcel_Shared_Excel5::getDistanceX($this->phpSheet, $startColumn, $startOffsetX, $endColumn, $endOffsetX);
                    $height = KDAPHPExcel_Shared_Excel5::getDistanceY($this->phpSheet, $startRow, $startOffsetY, $endRow, $endOffsetY);

                    // calculate offsetX and offsetY of the shape
                    $offsetX = $startOffsetX * KDAPHPExcel_Shared_Excel5::sizeCol($this->phpSheet, $startColumn) / 1024;
                    $offsetY = $startOffsetY * KDAPHPExcel_Shared_Excel5::sizeRow($this->phpSheet, $startRow) / 256;
            ...
            if ($this->version == self::XLS_BIFF8) {
                foreach ($this->sharedFormulaParts as $cell => $baseCell) {
                    ...
                    if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($column, $row, $this->phpSheet->getTitle())) {
                        ...
                        $this->phpSheet->getCell($cell)->setValueExplicit('=' . $formula, KDAPHPExcel_Cell_DataType::TYPE_FORMULA);
                    }
                }
            }
...
//                    echo '<br />';
            ...
            if ($definedName['isBuiltInName']) {
                ...
                if (count($explodes) == 2) {
                    if (($docSheet = $this->phpExcel->getSheetByName($explodes[0])) ||
                        ...
                        $this->phpExcel->addNamedRange(new KDAPHPExcel_NamedRange((string)$definedName['name'], $docSheet, $extractedRange, $localOnly, $scope));
                    }
                } else {
                    //    Named Value
    ...
    private function readRecordData($data, $pos, $len)
    {
        ...
        if ($this->encryption == self::MS_BIFF_CRYPTO_RC4) {
            ...
            throw new KDAPHPExcel_Reader_Exception('XOr encryption not supported');
        }
        return $recordData;
    }
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-time
                    $value = KDAPHPExcel_Shared_OLE::OLE2LocalDate(substr($this->summaryInformation, $secOffset + 4 + $offset, 8));
                    break;
...
//            echo 'Type is ', $type,', ';
            ...
            switch ($type) {
                ...
                case 0x40:    //    Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-Time
                    $value = KDAPHPExcel_Shared_OLE::OLE2LocalDate(substr($this->documentSummaryInformation, $secOffset + 4 + $offset, 8));
                    break;
                case 0x47:    //    Clipboard format
                    // not needed yet, fix later if necessary
                    break;
...
//        echo '<br />';
    ...
    private function readFilepass()
    {
        ...
        if ($length != 54) {
            throw new KDAPHPExcel_Reader_Exception('Unexpected file pass record length');
        }
        ...
        if (!$this->verifyPassword('VelvetSweatshop', substr($recordData, 6, 16), substr($recordData, 22, 16), substr($recordData, 38, 16), $this->md5Ctxt)) {
            throw new KDAPHPExcel_Reader_Exception('Decryption password incorrect');
        }
     ...
     *
     * @return KDAPHPExcel_Reader_Excel5n_RC4
     */
    private function makeKey($block, $valContext)
    {
        ...
        $md5 = new KDAPHPExcel_Reader_Excel5n_MD5();
        $md5->add($pwarray);

        $s = $md5->getContext();
        return new KDAPHPExcel_Reader_Excel5n_RC4($s);
    }
    ...
    private function readCodepage()
    {
        ...
        $this->codepage = KDAPHPExcel_Shared_CodePage::NumberToName($codepage);
    }
    ...
    private function readXf()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $objStyle->getProtection()->setLocked($isLocked ? KDAPHPExcel_Style_Protection::PROTECTION_INHERIT : KDAPHPExcel_Style_Protection::PROTECTION_UNPROTECTED);

            // bit 1; mask 0x02; 1 = Formula is hidden
            $isHidden = (0x02 & $xfTypeProt) >> 1;
            $objStyle->getProtection()->setHidden($isHidden ? KDAPHPExcel_Style_Protection::PROTECTION_PROTECTED : KDAPHPExcel_Style_Protection::PROTECTION_UNPROTECTED);
            ...
            switch ($horAlign) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    $objStyle->getAlignment()->setHorizontal(KDAPHPExcel_Style_Alignment::HORIZONTAL_FILL);
                    break;
                case 5:
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                if ($bordersLeftStyle = KDAPHPExcel_Reader_Excel5n_Style_Border::lookup((0x0000000F & self::getInt4d($recordData, 10)) >> 0)) {
                    $objStyle->getBorders()->getLeft()->setBorderStyle($bordersLeftStyle);
                }
                // bit: 7-4; mask: 0x000000F0; right style
                if ($bordersRightStyle = KDAPHPExcel_Reader_Excel5n_Style_Border::lookup((0x000000F0 & self::getInt4d($recordData, 10)) >> 4)) {
                    $objStyle->getBorders()->getRight()->setBorderStyle($bordersRightStyle);
                }
                // bit: 11-8; mask: 0x00000F00; top style
                if ($bordersTopStyle = KDAPHPExcel_Reader_Excel5n_Style_Border::lookup((0x00000F00 & self::getInt4d($recordData, 10)) >> 8)) {
                    $objStyle->getBorders()->getTop()->setBorderStyle($bordersTopStyle);
                }
                // bit: 15-12; mask: 0x0000F000; bottom style
                if ($bordersBottomStyle = KDAPHPExcel_Reader_Excel5n_Style_Border::lookup((0x0000F000 & self::getInt4d($recordData, 10)) >> 12)) {
                ...
                if ($bordersDiagonalStyle = KDAPHPExcel_Reader_Excel5n_Style_Border::lookup((0x01E00000 & self::getInt4d($recordData, 14)) >> 21)) {
                    $objStyle->getBorders()->getDiagonal()->setBorderStyle($bordersDiagonalStyle);
                }

                // bit: 31-26; mask: 0xFC000000 fill pattern
                if ($fillType = KDAPHPExcel_Reader_Excel5n_Style_FillPattern::lookup((0xFC000000 & self::getInt4d($recordData, 14)) >> 26)) {
                ...
                $objStyle->getFill()->setFillType(KDAPHPExcel_Reader_Excel5n_Style_FillPattern::lookup((0x003F0000 & $borderAndBackground) >> 16));

                // bit: 24-22; mask: 0x01C00000; bottom line style
                $objStyle->getBorders()->getBottom()->setBorderStyle(KDAPHPExcel_Reader_Excel5n_Style_Border::lookup((0x01C00000 & $borderAndBackground) >> 22));
                ...
                $objStyle->getBorders()->getTop()->setBorderStyle(KDAPHPExcel_Reader_Excel5n_Style_Border::lookup((0x00000007 & $borderLines) >> 0));

                // bit: 5-3; mask: 0x00000038; left line style
                $objStyle->getBorders()->getLeft()->setBorderStyle(KDAPHPExcel_Reader_Excel5n_Style_Border::lookup((0x00000038 & $borderLines) >> 3));

                // bit: 8-6; mask: 0x000001C0; right line style
                $objStyle->getBorders()->getRight()->setBorderStyle(KDAPHPExcel_Reader_Excel5n_Style_Border::lookup((0x000001C0 & $borderLines) >> 6));
    ...
    private function readSheet()
    {
        ...
        switch (ord($recordData[4])) {
            case 0x00:
                $sheetState = KDAPHPExcel_Worksheet::SHEETSTATE_VISIBLE;
                break;
            case 0x01:
                $sheetState = KDAPHPExcel_Worksheet::SHEETSTATE_HIDDEN;
                break;
            case 0x02:
                $sheetState = KDAPHPExcel_Worksheet::SHEETSTATE_VERYHIDDEN;
                break;
            default:
                $sheetState = KDAPHPExcel_Worksheet::SHEETSTATE_VISIBLE;
                break;
    ...
    private function readHorizontalPageBreaks()
    {
        ...
        if ($this->version == self::XLS_BIFF8 && !$this->readDataOnly) {
            ...
            for ($i = 0; $i < $nm; ++$i) {
                ...
                $this->phpSheet->setBreakByColumnAndRow($cf, $r, KDAPHPExcel_Worksheet::BREAK_ROW);
            }
        }
    }
    ...
    private function readVerticalPageBreaks()
    {
        ...
        if ($this->version == self::XLS_BIFF8 && !$this->readDataOnly) {
            ...
            for ($i = 0; $i < $nm; ++$i) {
                ...
                $this->phpSheet->setBreakByColumnAndRow($c, $rf, KDAPHPExcel_Worksheet::BREAK_COLUMN);
            }
        }
    }
    ...
    private function readPageSetup()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if (!$isNotInit) {
                ...
                switch ($isPortrait) {
                    case 0:
                        $this->phpSheet->getPageSetup()->setOrientation(KDAPHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE);
                        break;
                    case 1:
                        $this->phpSheet->getPageSetup()->setOrientation(KDAPHPExcel_Worksheet_PageSetup::ORIENTATION_PORTRAIT);
                        break;
                }
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if (($fmtRuns = $this->sst[$index]['fmtRuns']) && !$this->readDataOnly) {
                ...
                for ($i = 0; $i <= $sstCount; ++$i) {
                    if (isset($fmtRuns[$i])) {
                        $text = KDAPHPExcel_Shared_String::Substring($this->sst[$index]['value'], $charPos, $fmtRuns[$i]['charPos'] - $charPos);
                        $charPos = $fmtRuns[$i]['charPos'];
                    } else {
                        $text = KDAPHPExcel_Shared_String::Substring($this->sst[$index]['value'], $charPos, KDAPHPExcel_Shared_String::CountCharacters($this->sst[$index]['value']));
                    }
                ...
                if ($this->readEmptyCells || trim($this->sst[$index]['value']) !== '') {
                    $cell = $this->phpSheet->getCell($columnString . ($row + 1));
                    $cell->setValueExplicit($this->sst[$index]['value'], KDAPHPExcel_Cell_DataType::TYPE_STRING);
                    $emptyCell = false;
                }
            }
    ...
    private function readFormula()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if ((ord($recordData[6]) == 0) && (ord($recordData[12]) == 255) && (ord($recordData[13]) == 255)) {
                ...
                $value = KDAPHPExcel_Reader_Excel5n_ErrorCode::lookup(ord($recordData[8]));
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            switch ($isError) {
                case 0: // boolean
                    ...
                    break;
                case 1: // error type
                    $value = KDAPHPExcel_Reader_Excel5n_ErrorCode::lookup($boolErr);
...
//        echo '<br />';
    ...
    private function readWindow2()
    {
        ...
        if ($this->phpSheet->getSheetView()->getView() !== KDAPHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT) {
            //NOTE: this setting is inferior to page layout view(Excel2007-)
            $view = $isPageBreakPreview ? KDAPHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW : KDAPHPExcel_Worksheet_SheetView::SHEETVIEW_NORMAL;
    ...
    private function readPageLayoutView()
    {
        ...
        if ($fPageLayoutView === 1) {
            $this->phpSheet->getSheetView()->setView(KDAPHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT);
    ...
    private function readPane()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($this->frozen) {
                // frozen panes
                $this->phpSheet->freezePane(KDAPHPExcel_Cell::stringFromColumnIndex($px) . ($py + 1));
            } else {
    ...
    private function readDataValidation()
    {
        ...
        switch ($type) {
            case 0x00:
                $type = KDAPHPExcel_Cell_DataValidation::TYPE_NONE;
                break;
            case 0x01:
                $type = KDAPHPExcel_Cell_DataValidation::TYPE_WHOLE;
                break;
            case 0x02:
                $type = KDAPHPExcel_Cell_DataValidation::TYPE_DECIMAL;
                break;
            case 0x03:
                $type = KDAPHPExcel_Cell_DataValidation::TYPE_LIST;
                break;
            case 0x04:
                $type = KDAPHPExcel_Cell_DataValidation::TYPE_DATE;
                break;
            case 0x05:
                $type = KDAPHPExcel_Cell_DataValidation::TYPE_TIME;
                break;
            case 0x06:
                $type = KDAPHPExcel_Cell_DataValidation::TYPE_TEXTLENGTH;
                break;
            case 0x07:
                $type = KDAPHPExcel_Cell_DataValidation::TYPE_CUSTOM;
                break;
        }
        ...
        switch ($errorStyle) {
            case 0x00:
                $errorStyle = KDAPHPExcel_Cell_DataValidation::STYLE_STOP;
                break;
            case 0x01:
                $errorStyle = KDAPHPExcel_Cell_DataValidation::STYLE_WARNING;
                break;
            case 0x02:
                $errorStyle = KDAPHPExcel_Cell_DataValidation::STYLE_INFORMATION;
                break;
        }
        ...
        switch ($operator) {
            case 0x00:
                $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_BETWEEN;
                break;
            case 0x01:
                $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_NOTBETWEEN;
                break;
            case 0x02:
                $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_EQUAL;
                break;
            case 0x03:
                $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_NOTEQUAL;
                break;
            case 0x04:
                $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_GREATERTHAN;
                break;
            case 0x05:
                $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_LESSTHAN;
                break;
            case 0x06:
                $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_GREATERTHANOREQUAL;
                break;
            case 0x07:
                $operator = KDAPHPExcel_Cell_DataValidation::OPERATOR_LESSTHANOREQUAL;
                break;
        }
        ...
        foreach ($cellRangeAddresses as $cellRange) {
            ...
            foreach (KDAPHPExcel_Cell::extractAllCellReferencesInRange($stRange) as $coordinate) {
        ...
        if (!$this->readDataOnly) {
            ...
            switch ($sz) {
                case 0x14:
                    ...
                    $color = KDAPHPExcel_Reader_Excel5n_Color::map($colorIndex, $this->palette, $this->version);
    ...
    private function getNextToken($formulaData, $baseCell = 'A1')
    {
        ...
        switch ($id) {
            ...
            case 0x1C:    //    error code
                ...
                $name = 'tErr';
                $size = 2;
                $data = KDAPHPExcel_Reader_Excel5n_ErrorCode::lookup(ord($formulaData[1]));
                break;
            case 0x1D:    //    boolean
    ...
    private function readBIFF8CellAddress($cellAddressStructure)
    {
        ...
        $column = KDAPHPExcel_Cell::stringFromColumnIndex(0x00FF & self::getInt2d($cellAddressStructure, 2));
    ...
    private function readBIFF8CellRangeAddress($subData)
    {
        ...
        // bit: 7-0; mask 0x00FF; column index
        $fc = KDAPHPExcel_Cell::stringFromColumnIndex(0x00FF & self::getInt2d($subData, 4));
        ...
        // bit: 7-0; mask 0x00FF; column index
        $lc = KDAPHPExcel_Cell::stringFromColumnIndex(0x00FF & self::getInt2d($subData, 6));
    ...
    private static function readBIFF8Constant($valueData)
    {
        ...
        switch ($identifier) {
            ...
            case 0x10: // error code
                // offset: 1; size: 1; error code
                $value = KDAPHPExcel_Reader_Excel5n_ErrorCode::lookup(ord($valueData[1]));
                $size = 9;
                break;
        }
        return array(
    ...
    {
        return KDAPHPExcel_Shared_String::ConvertEncoding($string, 'UTF-8', $this->codepage);
    }
