...
class SMail
{
	...
	public function __construct($params=array())
	{
		...
		if(preg_match('/[^A-Za-z0-9\-\.]/', $arEmailParts[1]))
		{
			...
			if(class_exists('\idna_convert'))
			{
				...
				if(!IUtils::DetectUTF8($oldHost)) $oldHost = \CKDAImportUtils::Win1251Utf8($oldHost);
				$this->params['EMAIL'] = str_replace($arEmailParts[1], $idn->encode($oldHost), $this->params['EMAIL']);
			}
		}
	}
	
	public function CheckParams($maxTime=10)
	{
		if(!$this->paramsChecked)
		{
			...
			$maxTime = max(intval(ini_get('max_execution_time')) - 5, $maxTime);
			if($maxTime < 10) $maxTime = 10;
			if($maxTime > 50) $maxTime = 50;
			
			$this->imap = new \Bitrix\KdaImportexcel\Imap($this->params['SERVER'], $port, $tls, false, $this->params['EMAIL'], $this->params['PASSWORD'], $charset, $maxTime);
			$error = '';
	...
	public function GetFileId(&$arParams, $maxTime = 0, $extId = '')
	{
		if($this->CheckParams($maxTime))
		{
			...
			while(!empty($arFolders) && !$fid)
			{
				...
				$error='';
				$mailsIds = $this->imap->getSearch($folder, $arCriterias, $maxTime, $error);
				if(!empty($mailsIds))
				{
					$break = false;
					$i = count($mailsIds) - 1;
					while($i>=0 && !$break)
					{
						...
						if(($arMailFile = $this->imap->getMessageFile($folder, $mailId, $this->params, $allowExt, $error))!==false)
						{
							...
							if((!$this->params['LAST_DATE'] && strtotime($arMailFile['DATE']) > $time) || ($arMailFile['DATE']!=$this->params['LAST_DATE'] && strtotime($arMailFile['DATE']) > strtotime($this->params['LAST_DATE'])))
							{
						...
						elseif($this->params['FILENAME_REGEXP']=='Y' && (strpos($this->params['FILENAME'], '//')!==false || stripos($this->params['FILENAME'], 'href=')!==false) && ($arMailData = $this->imap->getMessage($folder, $mailId, 'DATA', $error))!==false)
						{
							if(preg_match('/'.str_replace('/', '\/', $arCriterias['FILENAME']).'/is', $arMailData['TEXT'], $m))
							{
								...
								if(!$this->params['LAST_DATE'] || $arMailData['DATE']!=$this->params['LAST_DATE'])
								{
									...
									elseif(!preg_match('/^\s*(ftp|http)/i', $path) && preg_match('/((ftp|http)s?:\/\/[^\s<>"]+)/i', $path, $m2)) $path = $m2[1];
		...
		if($arParams['NOTIFY_SUPPLIER']=='Y' && $arParams['NOTIFY_SUPPLIER_TEMPLATE_ID'] > 0 && ($days = (int)trim($arParams['NOTIFY_SUPPLIER_DAYS'])) > 0 && $arParams['LAST_DATE'])
		{
			if(time() - strtotime($arParams['LAST_DATE']) > $days*24*60*60 
				&& (!$arParams['LAST_DATE_NOTIFY'] || (time() - strtotime($arParams['LAST_DATE_NOTIFY']) > $days*24*60*60)))
			{
				$oProfile = \CKDAImportProfile::getInstance();
				$profileName = '';
				if(preg_match('/\D(\d+)\s*$/', $extId, $m) && ($arProfile = $oProfile->GetFieldsByID($m[1])))
				{
					$profileName = $arProfile['NAME'];
				}
				$arMailFields = array(
					'PROFILE_NAME' => $profileName,
					'EMAIL_TO' => $arParams['EMAIL'],
				);
				\CEvent::Send('KDA_IMPORT_SUPPLIER_NOTIFY', $oProfile->GetDefaultSiteId(), $arMailFields, 'Y', $arParams['NOTIFY_SUPPLIER_TEMPLATE_ID']);
				$arParams['LAST_DATE_NOTIFY'] = date('c');
			}
		}
	...
	public static function GetNewFile(&$json, $maxTime = 0, $extId='')
	{
		...
		if(!is_array($arParams)) $arParams = \KdaIE\Utils::Unserialize($json);
