...
class Imap
{
	...
	public function __construct($host, $port, $tls, $strict, $login, $password, $encoding, $timeout=10)
	{
	 ...
	 * @return int|false
	 */
	public function getSearch($mailbox, $arCriterias, $maxTime, &$error)
	{
		$error = null;
		...
		//if($arCriterias['SENTSINCE']) $command .= sprintf(' SENTSINCE "%s"', $arCriterias['SENTSINCE']);
		...
		{
			if(strpos($this->options['host'], 'yandex')!==false || !function_exists('mb_strlen'))
			{
		...
		if ($error)
		{
			if(($error == Imap::ERR_COMMAND_REJECTED || $error == Imap::ERR_EMPTY_RESPONSE) && !isset($this->recSearch))
			{
				$this->recSearch = true;
				$i = 0;
				$res = false;
				...
				while($i < $loops && ($res = $this->getSearch($mailbox, $arCriterias, $maxTime, $error))===false)
				{
					usleep($delay);
					$i++;
				}
		...
		$extSearch = false;
		if($this->SupportExtCriteria() && count($arUids)==0)
		{
			if(strlen($arCriterias['SUBJECT']) > 0 && strpos($arCriterias['SUBJECT'], '-')!==false)
			{
				$arCriterias2 = $arCriterias;
				unset($arCriterias2['SUBJECT']);
				$extSearch = true;
				$arUids = $this->getSearch($mailbox, $arCriterias2, $maxTime, $error);
				if(!is_array($arUids)) return $arUids;
			}
			elseif(strlen($arCriterias['SINCE']) > 0)
			{
				$arCriterias2 = $arCriterias;
				unset($arCriterias2['SINCE']);
				$extSearch = true;
				$arUids = $this->getSearch($mailbox, $arCriterias2, $maxTime, $error);
				if(!is_array($arUids)) return $arUids;
				$arUids = array_slice($arUids, -10);
			}
		}

		if((!$this->SupportExtCriteria() || $extSearch) && (strlen($arCriterias['FROM']) > 0 || strlen($arCriterias['SUBJECT']) > 0))
		{
			$i = count($arUids) - 1;
			while($i>=0)
			{
	...
	public function listMailboxes($mailbox, &$error)
	{
		...
		foreach ($this->getUntagged($regex, true) as $item)
		{
			...
			if (preg_match('/^ \{ ( \d+ ) \} $/ix', $sname, $literal))
				$sname = IUtils::binSubstr($item, IUtils::BinStrlen($matches[0]), $literal[1]);
	...
	public function getMessage($mailbox, $id, $section, &$error)
	{
		...
		foreach ($this->getUntagged($regex, true) as $item)
		{
			...
			{
				$sbody = IUtils::binSubstr($data, $literal[0][1]+IUtils::BinStrlen($literal[0][0]), $literal['size'][0]);
				continue;
			}
		}
	...
	protected function exchange($data, &$error)
	{
		...
		if (!preg_match(sprintf('/^ %s \x20 OK /ix', $this->getUTag()), $response))
		{
			if (preg_match(sprintf('/^ %s \x20 ( NO | BAD ) /ix', $this->getUTag()), $response))
				$error = Imap::ERR_COMMAND_REJECTED;
			else
				$error = Imap::ERR_BAD_SERVER;
		}
		
		//file_put_contents(__FILE__.'.log.txt', date('Y-m-d H:i:s')."\r\n".'request: '.$data."\r\n".'response: '.$response."\r\n".print_r($this->sessUntagged, true)."\r\n\r\n-------------------\r\n\r\n", FILE_APPEND);

		return preg_replace(sprintf('/^ %s \x20 /ix', $this->getUTag()), '', $response);
		return $response;
	}
...
//echo '< ', $line;
	...
	protected static function prepareString($data)
	{
		if (preg_match('/^[^\x00\x0a\x0d\x80-\xff]*$/', $data))
			...
			return sprintf("{%u}\x00%s", IUtils::BinStrlen($data), $data);
	}
