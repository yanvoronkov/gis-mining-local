...
if($_POST)
{
	...
	//2 step load sheets settings
	if(array_key_exists('SHEET_INDEXES', $_POST))
	{
		if(isset($SETTINGS)) unset($SETTINGS);
		if(isset($EXTRASETTINGS)) unset($EXTRASETTINGS);
	}
...
if(isset($_FILES) && is_array($_FILES))
{
	...
	if(isset($_FILES['TEMPLATE_FILE']) && $_FILES['TEMPLATE_FILE']['error']==0 && $_FILES['TEMPLATE_FILE']['size'] > 0 && preg_match('/\.xls[xm]$/i', $_FILES['TEMPLATE_FILE']['name']))
	{
...
if(strlen($PROFILE_ID) > 0 && $PROFILE_ID!=='new')
{
	...
	{
		$arFilterKeys = array('EFILTER', 'OFILTER', 'SFILTER');
...
function ShowSheetData($list, $SETTINGS, $SETTINGS_DEFAULT, $EXTRASETTINGS, $MODULE_RIGHT)
{
	...
	if($SETTINGS['SHOW_ONLY_SECTION_PROPERTY'][$list]=='Y' || $SETTINGS['SHOW_ONLY_SECTION_PROPERTY_WO_IBLOCK'][$list]=='Y')
	{
		$arFieldParams = array(
			...
			'EFILTER' => $SETTINGS['EFILTER'][$list],
			'SFILTER' => $SETTINGS['SFILTER'][$list]
		);
	}
	...
	$ee = CKDAExportExcel::getInstance($params, $EXTRASETTINGS, false, $PROFILE_ID);
	...
	$arSortElem = array();
	foreach(explode(',', $sortVal) as $sortParam)
	{
		list($sortBy, $sortOrder) = explode('=>', $sortParam);
		$arSortElem[$sortBy] = $sortOrder;
	}
	$arSortOffer = array();
	foreach(explode(',', $sortOfferVal) as $sortParam)
	{
		list($sortByOffer, $sortOrderOffer) = explode('=>', $sortParam);
		$arSortOffer[$sortByOffer] = $sortOrderOffer;
	}
	...
	<div class="kda-ee-additional-settings">
		...
		<div class="addsettings_inner">
			<table class="additional">
				...
				<tr>
					<td><?echo GetMessage("KDA_EE_HIDE_LIST"); ?>:</td>
					<td>
						<input type="hidden" name="SETTINGS[HIDE_SHEET][<?echo $list;?>]" value="N">
						<input type="checkbox" name="SETTINGS[HIDE_SHEET][<?echo $list;?>]" value="Y" <?if($SETTINGS['HIDE_SHEET'][$list]=='Y'){echo 'checked';}?>>
					</td>
				</tr>
				<tr>
	...
	echo '<th title="'.sprintf(GetMessage("KDA_EE_ROW_NUMBER"), $rowTitleIndex).'">'.($MODULE_RIGHT > 'T' ? '<span class="sandwich" data-key="COLUMN_TITLES" title="'.GetMessage("KDA_EE_ACTIONS_BTN").'"></span>' : '').'</th>';
	foreach($arFields as $k=>$field)
	{
		...
		if($isSortable)
		{
			$arSort = (strpos($field, 'OFFER_')===0 ? $arSortOffer : $arSortElem);
			$sortClass = 'sort_up';
			$sortTitle = GetMessage("KDA_EE_SETTINGS_SORT_ASC");
			if(array_key_exists($field, $arSort))
			{
				if($arSort[$field]!='DESC')
				{
					$sortClass = 'sort_down';
		...
		echo '<th class="kda-ee-field-select">'.
				...
				'<div>'.
					...
					($MODULE_RIGHT > 'T' && $isSortable ? '<a href="javascript:void(0);" class="'.$sortClass.'" onclick="EList.Sort(this, \''.(strpos($field, 'OFFER_')===0 ? '_OFFER' : '').'\');" title="'.htmlspecialcharsbx($sortTitle).'"></a>' : '').
					($MODULE_RIGHT > 'T' && $isSortable && strpos($sortClass, 'active')!==false ? '<a href="javascript:void(0);" class="sort_delete" onclick="EList.Sort(this, \''.(strpos($field, 'OFFER_')===0 ? '_OFFER' : '').'\', true);" title="'.htmlspecialcharsbx(GetMessage("KDA_EE_SETTINGS_SORT_DELETE")).'"></a>' : '').
				'</div>'.
			'</th>';
	}
	echo '</tr>';
...
if ($REQUEST_METHOD == "POST" && $MODE=='AJAX')
{
	...
	{
		$oFilter = new CKDAEEFilter($_POST['IBLOCK_ID']);
	...
	if($ACTION=='GET_UID')
	{
		...
		ob_end_clean();
		echo \KdaIE\Utils::PhpToJSObject(array('isOffers'=>($OFFERS_IBLOCK_ID > 0 ? 1 : 0)));
		die();
	}
...
?>
	...
	?>
			...
			<td width="60%" class="kda-ie-file-choose">
				<select name="SETTINGS_DEFAULT[FILE_EXTENSION]" id="kda-ee-file-extension"<?if($MODULE_RIGHT <= 'T'){echo ' disabled';}?>>
					<?if($xlsxShow){?>
						...
						<option value="xlsm" <?if($SETTINGS_DEFAULT['FILE_EXTENSION']=='xlsm'){echo 'selected';}?>>.XLSM</option>
					<?}?>
					...
					<option value="pdf" <?if($SETTINGS_DEFAULT['FILE_EXTENSION']=='pdf'){echo 'selected';}?>>.PDF</option>
					<?/*?><option value="html" <?if($SETTINGS_DEFAULT['FILE_EXTENSION']=='html'){echo 'selected';}?>>.HTML</option><?*/?>
				</select>
			</td>
		</tr>
		...
		<tr>
			<td><?echo GetMessage("KDA_EE_INFOBLOCK"); ?></td>
			<td>
				<?echo GetIBlockDropDownListEx($SETTINGS_DEFAULT['IBLOCK_ID'], 'SETTINGS_DEFAULT[IBLOCK_TYPE_ID]', 'SETTINGS_DEFAULT[IBLOCK_ID]', array('MIN_PERMISSION'=>'R'), '', '', 'class="adm-detail-iblock-types"'.($MODULE_RIGHT <= 'T' ? ' disabled' : ''), 'class="adm-detail-iblock-list"'.($MODULE_RIGHT <= 'T' ? ' disabled' : '')); ?>
			</td>
		</tr>
		...
		<tr>
			<td><?echo GetMessage("KDA_EE_EXPORT_ADD_DOMAIN"); ?>: <span id="hint_EXPORT_ADD_DOMAIN"></span><script>BX.hint_replace(BX('hint_EXPORT_ADD_DOMAIN'), '<?echo GetMessage("KDA_EE_EXPORT_ADD_DOMAIN_HINT"); ?>');</script></td>
			<td>
				<?
				$obRequest = \Bitrix\Main\Context::getCurrent()->getRequest();
				$requestUri = trim($obRequest->getRequestUri());
				$placeholder = ($obRequest->isHttps() ? 'https' : 'http').'://'.$_SERVER['HTTP_HOST'];
				?>
				<table cellspacing="0"><tr>
				<td style="padding-left: 0px;"><input type="checkbox" name="SETTINGS_DEFAULT[EXPORT_ADD_DOMAIN]" value="Y" <?if($SETTINGS_DEFAULT['EXPORT_ADD_DOMAIN']=='Y'){echo 'checked';}?> onchange="var domainWrap = $('#export_add_domain_value'), domainInput = $('input[type=text]', domainWrap); if(this.checked){domainWrap.show(); if(domainInput.val().length==0){domainInput.val(window.location.protocol+'//'+window.location.host);}}else{domainWrap.hide();}"></td>
				<td>&nbsp; &nbsp;</td>
				<td id="export_add_domain_value"<?if($SETTINGS_DEFAULT['EXPORT_ADD_DOMAIN']!='Y'){echo ' style="display: none;"';}?>>
					<input type="text" name="SETTINGS_DEFAULT[EXPORT_ADD_DOMAIN_VALUE]" value="<?=htmlspecialcharsbx($SETTINGS_DEFAULT['EXPORT_ADD_DOMAIN_VALUE'])?>" size="40" placeholder="<?echo $placeholder;?>">
				</td>
				</tr></table>
			</td>
		</tr>
		...
		<tr <?if($SETTINGS_DEFAULT['EXPORT_SEP_SECTIONS']!='Y'){echo 'style="display: none;"';}?>>
			...
			<td>
				<table cellspacing="0"><tr>
				<td style="padding-left: 0px;"><input type="checkbox" name="SETTINGS_DEFAULT[EXPORT_SECTION_PATH]" value="Y" <?if($SETTINGS_DEFAULT['EXPORT_SECTION_PATH']=='Y'){echo 'checked';}?> onchange="if(this.checked){$('#export_section_path_mode').show();}else{$('#export_section_path_mode').hide();}"></td>
				<td>&nbsp; &nbsp;</td>
				<td id="export_section_path_mode"<?if($SETTINGS_DEFAULT['EXPORT_SECTION_PATH']!='Y'){echo ' style="display: none;"';}?>>
					<select name="SETTINGS_DEFAULT[EXPORT_SECTION_PATH_MODE]">
						<option value="ALL" <?if($SETTINGS_DEFAULT['EXPORT_SECTION_PATH_MODE']=='ALL'){echo 'selected';}?>><?echo GetMessage("KDA_EE_EXPORT_SECTION_PATH_MODE_ALL"); ?></option>
						<option value="END" <?if($SETTINGS_DEFAULT['EXPORT_SECTION_PATH_MODE']=='END'){echo 'selected';}?>><?echo GetMessage("KDA_EE_EXPORT_SECTION_PATH_MODE_END"); ?></option>
					</select>
				</td>
				</tr></table>
			</td>
		...
		if($bCatalog)
		{
			...
			if(!is_array($arCatalogFields)) $arCatalogFields = array();
		}
		?>
			...
			<td>
				<table cellspacing="0"><tr>
					<td style="padding-left: 0px;">
						<input type="checkbox" name="SETTINGS_DEFAULT[EXPORT_ONE_OFFER_MIN_PRICE]" value="Y" <?if($SETTINGS_DEFAULT['EXPORT_ONE_OFFER_MIN_PRICE']=='Y'){echo 'checked';}?> onchange="EProfile.RadioChb(this, 'SETTINGS_DEFAULT[EXPORT_ONE_OFFER_MAX_PRICE]'); if(this.checked){$('#kda_ee_offerminpricetype').show();}else{$('#kda_ee_offerminpricetype').hide();} ">
					</td>
					<td>&nbsp; &nbsp;</td>
					<td id="kda_ee_offerminpricetype" <?if($SETTINGS_DEFAULT['EXPORT_ONE_OFFER_MIN_PRICE']!='Y'){echo 'style="display: none;"';}?>>
					<?
					if(is_array($arCatalogFields) && count($arCatalogFields) > 0)
					{
		...
		<tr class="kda-sku-block" <?if(!$OFFERS_IBLOCK_ID){echo 'style="display: none;"';}?> >
			<td><?echo GetMessage("KDA_EE_EXPORT_ONE_OFFER_MAX_PRICE"); ?>:</td>
			<td>
				<table cellspacing="0"><tr>
					<td style="padding-left: 0px;">
						<input type="checkbox" name="SETTINGS_DEFAULT[EXPORT_ONE_OFFER_MAX_PRICE]" value="Y" <?if($SETTINGS_DEFAULT['EXPORT_ONE_OFFER_MAX_PRICE']=='Y'){echo 'checked';}?> onchange="EProfile.RadioChb(this, 'SETTINGS_DEFAULT[EXPORT_ONE_OFFER_MIN_PRICE]'); if(this.checked){$('#kda_ee_offermaxpricetype').show();}else{$('#kda_ee_offermaxpricetype').hide();}">
					</td>
					<td>&nbsp; &nbsp;</td>
					<td id="kda_ee_offermaxpricetype" <?if($SETTINGS_DEFAULT['EXPORT_ONE_OFFER_MAX_PRICE']!='Y'){echo 'style="display: none;"';}?>>
					<?
					if(is_array($arCatalogFields) && count($arCatalogFields) > 0)
					{
						echo GetMessage("KDA_EE_EXPORT_ONE_OFFER_MAX_PRICE_TYPE").'<br>';
						?>
						<select name="SETTINGS_DEFAULT[EXPORT_ONE_OFFER_MAX_PRICE_TYPE]">
							<?
							foreach($arCatalogFields as $arCatalogField)
							{
								$priceCode = false;
								...
								elseif(preg_match('/^ICAT_PRICE(\d+)_PRICE$/', $arCatalogField['value'], $m)) $priceCode = 'CATALOG_PRICE_'.$m[1];
								if($priceCode===false) continue;
								?><option value="<?echo $priceCode;?>"<?if($SETTINGS_DEFAULT['EXPORT_ONE_OFFER_MAX_PRICE_TYPE']==$priceCode){echo ' selected';}?>><?echo $arCatalogField['name'];?></option><?
							}
							?>
						</select>
						<?
		...
		<tr>
			<td><?echo GetMessage("KDA_EE_DISPLAY_SETTING_GRIDLINES");?>:</td>
			<td>
				<select name="SETTINGS_DEFAULT[GRIDLINES]">
					<option value=""><?echo GetMessage("KDA_EE_DISPLAY_SETTING_GRIDLINES_SHOW"); ?></option>
					<option value="HIDE" <?if($SETTINGS_DEFAULT['GRIDLINES']=='HIDE'){echo 'selected';}?>><?echo GetMessage("KDA_EE_DISPLAY_SETTING_GRIDLINES_HIDE"); ?></option>
				</select>
			</td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("KDA_EE_DISPLAY_SETTING_MARGINS");?>:</td>
			<td>
				<select name="SETTINGS_DEFAULT[MARGINS]">
					<option value=""><?echo GetMessage("KDA_EE_DISPLAY_SETTING_MARGINS_STANDARD"); ?></option>
					<option value="NONE" <?if($SETTINGS_DEFAULT['MARGINS']=='NONE'){echo 'selected';}?>><?echo GetMessage("KDA_EE_DISPLAY_SETTING_MARGINS_NONE"); ?></option>
				</select>
			</td>
		</tr>
			...
			<td>
				...
				<td id="googless"<?if($SETTINGS_DEFAULT['EXPORT_TO_GOOGLE_SPREADSHEETS']!='Y'){echo ' style="display: none;"';}?>>
					<div style="margin-bottom: 7px;">
						...
						<div><input type="text" name="SETTINGS_DEFAULT[GOOGLE_TOKEN]" value="<?=htmlspecialcharsbx($SETTINGS_DEFAULT['GOOGLE_TOKEN'])?>" size="50"> &nbsp; <a href="https://esolutions.su/marketplace/oauth.php?get_token=1" target="_blank"><?echo GetMessage("KDA_EE_EXT_SERVICES_GOOGLE_TOKEN_GET");?></a> <?/*echo GetMessage("KDA_EE_EXT_SERVICES_GOOGLE_TOKEN_GET_MORE");*/?></div>
					</div>
					<div>
		...
		<tr>
			...
			?>
			<td><?echo GetMessage("KDA_EE_EXT_SERVICES_EMAIL");?>: <span id="hint_EXT_SERVICES_EMAIL"></span><script>BX.hint_replace(BX('hint_EXT_SERVICES_EMAIL'), '<?echo sprintf(GetMessage("KDA_EE_EXT_SERVICES_EMAIL_HINT"), $eventName, $eventToField); ?>');</script></td>
			<td>
				<table cellspacing="0"><tr>
		...
		<tr>
			<td><?echo GetMessage("KDA_EE_OTHER_SETTINGS_REMOVE_EMPTY_COLUMNS");?>:</td>
			<td>
				<input type="checkbox" name="SETTINGS_DEFAULT[REMOVE_EMPTY_COLUMNS]" value="Y" <?=htmlspecialcharsbx($SETTINGS_DEFAULT['REMOVE_EMPTY_COLUMNS']=='Y' ? 'checked' : '')?>>
			</td>
		</tr>
			...
			foreach($arKeys as $list)
			{
				...
				<div class="kda-ee-sheet-wrap<?if($ind > 0){echo ' withmargin';} if($MODULE_RIGHT <= 'T'){echo ' minrigths';}?>">
					...
					if($SETTINGS_DEFAULT['USE_NEW_FILTER']=='Y')
					{
						...
						$offersIblockId = CKDAExportUtils::GetOfferIblock($iblockId);
						?>
						...
						<div class="kda-ee-sheet-cfilter-wrap">
							<div class="kda-ee-sheet-cfilter-title-wrap">
								<span class="kda-ee-sheet-cfilter-title kda-ee-sheet-cfilter-title-active" onclick="EProfile.ChangeFilter(this)"><?echo GetMessage('KDA_EE_PF_FILTER_ELEMENTS')?></span>
								<?if($offersIblockId){?>
									<span class="kda-ee-sheet-cfilter-title" onclick="EProfile.ChangeFilter(this)"><?echo GetMessage('KDA_EE_PF_FILTER_OFFERS')?></span>
								<?}?>
								<span class="kda-ee-sheet-cfilter-title" onclick="EProfile.ChangeFilter(this)"><?echo GetMessage('KDA_EE_PF_FILTER_SECTIONS')?></span>
							</div>
							<div class="kda-ee-sheet-cfilter-wrap-inner">
								<?								
								$eFilter = new CKDAEEFilter($iblockId);
								$eFilter->ShowFilterBlock('kda-ee-sheet-efilter-'.$list, (isset($SETTINGS['EFILTER'][$list]) ? $SETTINGS['EFILTER'][$list] : array()), $fl);

								if($offersIblockId)
								{
									$oFilter = new CKDAEEFilter($offersIblockId, 'o');
									$oFilter->ShowFilterBlock('kda-ee-sheet-ofilter-'.$list, (isset($SETTINGS['OFILTER'][$list]) ? $SETTINGS['OFILTER'][$list] : array()), $fl);
								}
								
								$eFilter = new CKDAEEFilter($iblockId, 's');
								$eFilter->ShowFilterBlock('kda-ee-sheet-sfilter-'.$list, (isset($SETTINGS['SFILTER'][$list]) ? $SETTINGS['SFILTER'][$list] : array()), $fl);
								?>
					...
					<div class="kda-ee-sheet" 
						...
						if($SETTINGS_DEFAULT['USE_NEW_FILTER']=='Y')
						{
							//ShowSheetData($list, $SETTINGS, $SETTINGS_DEFAULT, $EXTRASETTINGS, $MODULE_RIGHT);
						}
						?>
					</div>
					<?
		 ...
		 <table width="100%"><tr><td width="50%">
			...
			<div class="detail_status">
				...
				echo CAdminMessage::ShowMessage(array(
					...
					' <span id="kda_ee_ready_file" style="visibility: hidden;"><br>'.GetMessage("KDA_EE_READY_FILE_LINK").' <br><a href="'.htmlspecialcharsbx($outputFile).'?hash='.md5(mt_rand()).'" target="_blank">'.$outputFile.'</a> '.($GLOBALS['USER']->CanDoFileOperation('fm_download_file', array('', htmlspecialcharsbx($outputFile))) ? '<br><br><a href="/bitrix/admin/fileman_file_download.php?path='.htmlspecialcharsbx($outputFile).'&lang='.LANGUAGE_ID.'&hash='.md5(mt_rand()).'">'.GetMessage("KDA_EE_DOWNLOAD_FILE").'</a>' : '').'</span><br>',
					"HTML" => true,
				));
				?>
			</div>
