...
$profileType = \Bitrix\KdaImportexcel\ProfileGroupTable::TYPE_IMPORT_IBLOCK;
...
if($_POST['action']=='get_profile_run_form' && strlen($_POST['profile_id']) > 0)
{
	...
	$oProfile = \CKDAImportProfile::getInstance($suffix);
	...
	echo '<form method="post" action="/bitrix/admin/'.$moduleFilePrefix.'.php?PROFILE_ID='.$pid.'&lang='.LANGUAGE_ID.'">'.
		...
		'<input type="submit" value="'.GetMessage('KDA_IE_RUN_IMPORT').'">'.
	'</form>';
	die();
}
...
if($lAdmin->EditAction())
{
	foreach ($_POST['FIELDS'] as $ID => $arFields)
	{
		...
		if($bUserIsAdmin)
		{
			if(array_key_exists('RIGHTS', $arFields))
			{
				$arOldRights = $arNewRights = array();
				$arRightsFields = array('PROFILE_ID'=>$ID, 'PROFILE_TYPE'=>\Bitrix\KdaImportexcel\ProfileRightsTable::TYPE_IMPORT_IBLOCK);
				$dbRes = \Bitrix\KdaImportexcel\ProfileRightsTable::getlist(array('filter'=>$arRightsFields, 'order'=>array('ID'=>'ASC')));
				while($arr = $dbRes->Fetch())
				{
					$arOldRights[] = $arr;
				}
				
				if($arFields['RIGHTS']=='OWNER')
				{
					$arItemRights = array_merge($arRightsFields, array('OWNER'=>'Y', 'GROUP_ID'=>false));
					if(count($arOldRights) > 0) $arItemRights = array_merge(array_shift($arOldRights), $arItemRights);
					$arNewRights[] = $arItemRights;
				}
				elseif($arFields['RIGHTS']=='GROUPS')
				{
					if(is_array($arFields['RIGHTS_GROUPS']))
					{
						foreach($arFields['RIGHTS_GROUPS'] as $groupId)
						{
							$arItemRights = array_merge($arRightsFields, array('OWNER'=>'N', 'GROUP_ID'=>$groupId));
							if(count($arOldRights) > 0) $arItemRights = array_merge(array_shift($arOldRights), $arItemRights);
							$arNewRights[] = $arItemRights;
						}
					}
				...
				foreach($arNewRights as $arItemRights)
				{
					if(isset($arItemRights['ID'])) \Bitrix\KdaImportexcel\ProfileRightsTable::update($arItemRights['ID'], $arItemRights);
					else \Bitrix\KdaImportexcel\ProfileRightsTable::add($arItemRights);
				}
				foreach($arOldRights as $arItemRights)
				{
					\Bitrix\KdaImportexcel\ProfileRightsTable::delete($arItemRights['ID']);
				}
			}
		}
		else
		{
			if(array_key_exists('OWNER_ID', $arFields)) unset($arFields['OWNER_ID']);
		}
		
		if(array_key_exists('RIGHTS', $arFields)) unset($arFields['RIGHTS']);
		if(array_key_exists('RIGHTS_GROUPS', $arFields)) unset($arFields['RIGHTS_GROUPS']);
...
$getListParams = array(
	'select' => array(
		...
		//'RIGHTS_OWNER'=>'PROFILE_RIGHTS.OWNER'
		//'PROFILE_EXEC_ID'
	),
	/*'runtime' => array(
...
$oProfile->CheckRightsInFilter($getListParams['filter']);
...
$lAdmin->AddHeaders(array(
	...
	array("id"=>"OWNER", "content"=>GetMessage("KDA_IE_PL_OWNER"), "default"=>false),
	array("id"=>"RIGHTS", "content"=>GetMessage("KDA_IE_PL_RIGHTS"), "default"=>false),
...
$arUserGroups = array();
$dbRes = \Bitrix\Main\GroupTable::getList(array('order'=>array('ID'=>'ASC')));
while($arr = $dbRes->Fetch())
{
	if($arr['ANONYMOUS']=='Y') continue;
	$arUserGroups[$arr['ID']] = $arr;
}

$arProfiles = $arProfileIds = array();
...
$arProfileRights = array();
if(count($arProfileIds) > 0)
{
	$dbRes = \Bitrix\KdaImportexcel\ProfileRightsTable::getList(array('filter'=>array('PROFILE_ID'=>$arProfileIds, 'PROFILE_TYPE'=>\Bitrix\KdaImportexcel\ProfileRightsTable::TYPE_IMPORT_IBLOCK), 'select'=>array('PROFILE_ID', 'OWNER', 'GROUP_ID')));
	while($arr = $dbRes->Fetch())
	{
		$arProfileRights[$arr['PROFILE_ID']][$arr['GROUP_ID'] > 0 ? $arr['GROUP_ID'] : 0] = $arr;
	}
}
...
{
	$arProfile['ID'] = $f_ID = $arProfile['ID'] - 1;
	...
	$row->AddCheckField("ACTIVE", $arProfile['ACTIVE']);
	$row->AddViewField("NAME", '<a href="'.$moduleFilePrefix.'.php?PROFILE_ID='.$f_ID.'&lang='.LANG.'">'.$arProfile['NAME'].'</a>');
	$row->AddInputField("NAME", array('SIZE'=>40));
	$row->AddInputField("SORT", array('SIZE'=>10));
	$row->AddField("DATE_START", $arProfile['DATE_START']);
	$row->AddField("DATE_FINISH", $arProfile['DATE_FINISH']);
	
	$arStatus = $oProfile->GetStatus($f_ID);
	$row->AddField("STATUS", '<div style="min-width: 100px">'.(strlen($arStatus['MESSAGE']) > 0 ? $arStatus['MESSAGE'] : '').'</div>');
	
	$row->AddViewField("OWNER", ($arProfile['OWNER_ID'] > 0 ? '['.$arProfile['OWNER_ID'].'] '.$arProfile['OWNER_LOGIN'] : ''));
	
	$row->AddField("RUN_BTN", '<a href="javascript:void(0)" class="adm-nav-page esol_ix_run_btn" onclick="EProfileList.RunProfileImport(\''.$f_ID.'\');">'.GetMessage('KDA_IE_RUN_IMPORT').'</a>');
	
	$rights = $rightsVal = '';
	if(!is_array($arProfileRights[$f_ID+1])) $arProfileRights[$f_ID+1] = array();
	foreach($arProfileRights[$f_ID+1] as $arItemRights)
	{
		if($arItemRights['OWNER']=='Y')
		{
			$rights = (strlen($rights) > 0 ? $rights.'<br>' : '').GetMessage("KDA_IE_PROFILE_RIGHTS_OWNER");
			$rightsVal = 'OWNER';
		}
		elseif($arItemRights['GROUP_ID'] > 0)
		{
			$rights = (strlen($rights) > 0 ? $rights.'<br>' : '').'['.$arItemRights['GROUP_ID'].'] '.(isset($arUserGroups[$arItemRights['GROUP_ID']]) ? $arUserGroups[$arItemRights['GROUP_ID']]['NAME'] : '');
			$rightsVal = 'GROUPS';
		}
	}
	if(strlen($rights)==0) $rights = GetMessage("KDA_IE_PROFILE_RIGHTS_SHARE");
	
	$row->AddViewField("RIGHTS", $rights);
	if($bUserIsAdmin)
	{
		$row->AddEditField("OWNER", '<div style="min-width: 100px">'.str_replace(array('BX.', 'BX(', 'document.'), array('window.parent.BX.', 'window.parent.BX(', 'window.parent.document.'), FindUserID('FIELDS['.($f_ID+1).'][OWNER_ID]', ($arProfile['OWNER_ID'] > 0 ? $arProfile['OWNER_ID'] : ''), '', 'form_'.$sTableID)).'</div>');
		
		$userGroups = '';
		foreach($arUserGroups as $arUserGroup)
		{
			$userGroups .= '<option value="'.htmlspecialcharsbx($arUserGroup['ID']).'"'.(array_key_exists($arUserGroup['ID'], $arProfileRights[$f_ID+1]) ? ' selected' : '').'>'.htmlspecialcharsbx('['.$arUserGroup['ID'].'] '.$arUserGroup['NAME']).'</option>';
		}
		$row->AddEditField("RIGHTS", '<select name="FIELDS['.($f_ID+1).'][RIGHTS]" onchange="document.getElementById(\'div_'.$sTableID.'_groups_'.($f_ID+1).'\').style.display = (this.value==\'GROUPS\' ? \'block\' : \'none\')">
			<option value="">'.GetMessage("KDA_IE_PROFILE_RIGHTS_SHARE").'</option>
			<option value="OWNER"'.($rightsVal=='OWNER' ? ' selected' : '').'>'.GetMessage("KDA_IE_PROFILE_RIGHTS_OWNER").'</option>
			<option value="GROUPS"'.($rightsVal=='GROUPS' ? ' selected' : '').'>'.GetMessage("KDA_IE_PROFILE_RIGHTS_GROUPS").'</option>
		</select><div id="div_'.$sTableID.'_groups_'.($f_ID+1).'" style="display: '.($rightsVal=='GROUPS' ? 'block' : 'none').'"><select name="FIELDS['.($f_ID+1).'][RIGHTS_GROUPS][]" multiple>'.$userGroups.'</select></div>');
	}
	...
	if(true /*$arProfile['PROFILE_EXEC_ID'] > 0*/)
	{
