	...
	if(is_array($arPost['NAMES']))
	{
		foreach($arPost['NAMES'] as $propIndex)
		{
			...
			else
			{
				...
				if($arPropFields['PROPERTY_TYPE']=='S' && $arPropFields['USER_TYPE']=='directory')
				{
					\CModule::IncludeModule('highloadblock');
					$hlName = ToLower(preg_replace('/[^A-Za-z0-9]+/', '', $arPropFields['CODE']));
					$hlData = array(
						'NAME' => 'Property'.ToUpper(mb_substr($hlName, 0, 1)).mb_substr($hlName, 1),
						'TABLE_NAME' => \CIBlockPropertyDirectory::createHighloadTableName('property_'.$hlName)
					);
					$result = \Bitrix\Highloadblock\HighloadBlockTable::add($hlData);
					
					if($result->isSuccess())
					{
						$highBlockID = $result->getId();
						$_POST["PROPERTY_USER_TYPE_SETTINGS"]["TABLE_NAME"] = $data['TABLE_NAME'];
						$arFieldsName = array('UF_NAME', 'UF_XML_ID', 'UF_LINK', 'UF_DESCRIPTION', 'UF_FULL_DESCRIPTION', 'UF_SORT', 'UF_FILE', 'UF_DEF');
						$obUserField = new \CUserTypeEntity();
						$intSortStep = 100;
						foreach($arFieldsName as $fieldName)
						{
							$fieldMandatory = 'N';
							switch($fieldName)
							{
							...
							if(GetMessage("KDA_IE_NP_".$fieldName))
							{
								$arUserField["EDIT_FORM_LABEL"] = $arUserField["LIST_COLUMN_LABEL"] = $arUserField["LIST_FILTER_LABEL"] = array(LANGUAGE_ID => GetMessage("KDA_IE_NP_".$fieldName));
							}
							$obUserField->Add($arUserField);
							$intSortStep += 100;
						}
					}
					$arPropFields['USER_TYPE_SETTINGS'] = array('TABLE_NAME'=>$hlData['TABLE_NAME']);
				}
			...
			<td class="adm-detail-content-cell-r">
				<select name="NAMES[]" multiple>
					...
					if(is_array($arPost['items']))
					{
						foreach($arPost['items'] as $arItem)
						{
							echo '<option value="'.htmlspecialcharsex($arItem['index']).'">'.htmlspecialcharsbx($arItem['text']).'</option>';
						}
					}
					?>
				</select>
