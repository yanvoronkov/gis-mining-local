...
$oProfileHl = new CKDAImportProfile('highload');
$arProfilesHl = $oProfileHl->GetList();
...
if(strlen($find_profile_id) > 0)
{
	if(strpos($find_profile_id, 'hl')===0)
	{
		$arFilter['PROFILE_ID'] = substr($find_profile_id, 2);
		$arFilter['PROFILE_TYPE'] = \Bitrix\KdaImportexcel\ProfileExecTable::TYPE_HLBLOCK;
	}
	else
	{
		$arFilter['PROFILE_ID'] = $find_profile_id;
		$arFilter['PROFILE_TYPE'] = \Bitrix\KdaImportexcel\ProfileExecTable::TYPE_IBLOCK;
	}
}
...
$getListParams = array(
	...
	'select'=>array(
		...
		'PROFILE_HL_NAME'=>'PROFILE_HL.NAME',		
...
while($db_res = $rsData->NavNext(true, "a_"))
{
	...
	$importFileLink = $moduleFilePrefix;
	if(strlen($a_PROFILE_NAME)==0 && strlen($a_PROFILE_HL_NAME) > 0)
	{
		$a_PROFILE_NAME = $a_PROFILE_HL_NAME.' (highload)';
		$importFileLink = $importFileLink.'_highload';
	}
	...
	$row->AddViewField("PROFILE_ID", '<a href="/bitrix/admin/'.$importFileLink.'.php?lang='.LANG.'&PROFILE_ID='.($a_PROFILE_ID - 1).'">'.$a_PROFILE_NAME.'</a>');
	...
	$arParams = \KdaIE\Utils::Unserialize($db_res['PARAMS']);
...
<tr>
	...
	<td>
		<select name="find_profile_id" >
			...
			if(count($arProfilesHl) > 0)
			{
				?><optgroup label="<?echo GetMessage("KDA_IE_EVENTLOG_HIGHLOAD_PROFILES")?>"><?
				foreach($arProfilesHl as $k=>$profile)
				{
					$key = 'hl'.($k + 1);
					?><option value="<?echo $key;?>" <?if($find_profile_id==$key){echo 'selected';}?>><?echo $profile; ?></option><?
				}
