...
{
	$arParams = \KdaIE\Utils::Unserialize(\Bitrix\Main\Config\Option::get('iblock', 'KDA_IBLOCK'.$IBLOCK_ID.'_PRICEQNT_CONFORMITY'));
...
$arParams1 = \KdaIE\Utils::Unserialize(\Bitrix\Main\Config\Option::get('iblock', 'KDA_IBLOCK'.$IBLOCK_ID.'_PRICEQNT_CONFORMITY'));
...
$arParams2 = array();
if($OFFERS_IBLOCK_ID) $arParams2 = \KdaIE\Utils::Unserialize(\Bitrix\Main\Config\Option::get('iblock', 'KDA_IBLOCK'.$OFFERS_IBLOCK_ID.'_PRICEQNT_CONFORMITY'));

require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_popup_admin.php");
?>
<form action="" method="post" enctype="multipart/form-data" name="field_settings" id="kda-ie-price-calculating-form">
...
while((($jk = ($j > 0 ? '_'.$j : '')) || true) && ($jp = 'PARAMS'.$jk) && ($jm = 'MAP'.$jk) && array_key_exists($jp, $arParams1))
{
	...
	if(!is_array($DEFAULTS)) $DEFAULTS = array();
?>	
 <div class="kda-ie-price-calculating-wrap" data-index="<?echo $j;?>"<?if($j > 0){echo 'style="display: none;"';}?>>
	<table width="100%" class="kda-ie-price-calculating">
		...
		<tr>
			...
			<td class="adm-detail-content-cell-r">
				<div class="kda-ie-price-calculating-ptypes"></div>
				<div class="kda-ie-price-calculating-ptype-new"><a href="javascript:void(0)" onclick="EProfile.AddNewCalcType(this)"><span><?echo GetMessage("KDA_IE_CALC_ADD_PRICE_TYPE"); ?></span></a></div>
			</td>
		</tr>
		<?}?>
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<select name="DEFAULTS<?echo $jk;?>[QUANTITY_CALC]">
						...
						<option value="NONE"<?if($DEFAULTS['QUANTITY_CALC']=='NONE'){echo ' selected';}?>><?echo GetMessage("KDA_IE_CALC_QUANTITY_NONE");?></option>
					</select>
				</td>
			</tr>
		<?}?>
