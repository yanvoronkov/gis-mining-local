...
function CheckCronDocRoot($path)
{
	...
	return (bool)($docRoot==$path || rtrim($docRootReal.KDA_EE_PATH2EXPORTS, '/')==rtrim(realpath($path.KDA_EE_PATH2EXPORTS), '/'));
}
...
$needSave = false;
if(preg_match_all("#^.*?".preg_quote($docRoot.KDA_EE_PATH2EXPORTS).$cronFrame." +(\d+) *>.*?$#im", $cfg_data, $m))
{
	$arIds = array();
	foreach($m[1] as $pid)
	{
	...
	foreach($m[1] as $pid)
	{
		...
		{
			$cfg_data = preg_replace("#^.*?".preg_quote($docRoot.KDA_EE_PATH2EXPORTS).$cronFrame." +".$pid." *>.*?$#im", "", $cfg_data);
			$needSave = true;
		}
	}
}
if($_REQUEST["action"]=="deleterecord" && $_REQUEST["key"])
{
	...
	{
		$cfg_data = preg_replace("#^\s*".preg_quote($time)."\s+.*?".preg_quote($docRoot.KDA_EE_PATH2EXPORTS).$cronFrame." +".preg_quote($pids)." *>.*?$#im", "", $cfg_data);
		$needSave = true;
	}*/
...
if ($_REQUEST["action"]=="save")
{
	...
	if (strlen($strErrorMessage)<=0 && $_REQUEST["subaction"]=='add')
	{
		...
		if (strlen($PROFILE_ID) > 0)
		{
			...
			if($_REQUEST["recordkey"])
			{
				...
				if(strlen($time) > 0 && strlen($pids) > 0)
				{
					$cfg_data = preg_replace("#^\s*".preg_quote($time)."\s+.*?".preg_quote($docRoot.KDA_EE_PATH2EXPORTS).$cronFrame." +".preg_quote($pids)." *>.*?$#im", trim($cfg_subdata), $cfg_data);
				}*/
			...
			else
			{
				...
				if($cronWritable && $_REQUEST["auto_cron_tasks"]=="Y")
				{
					...
					*/
					...
					$strSuccessMessage .= GetMessage("KDA_EE_CRON_SUCCESS_SAVE")."<br><br><i>".$cfg_subdata.'</i><br><br>';
					$strSuccessMessage .= GetMessage("KDA_EE_CRON_WHERE_IS")."<br>";
					$strSuccessMessage .= '<i>'.$strTime.'</i> - '.GetMessage("KDA_EE_CRON_TIME_EXECUTE_COMMENT")."<br>";
					$strSuccessMessage .= '<i>'.$agent_php_path.'</i> - '.GetMessage("KDA_EE_CRON_PHP_PATH_COMMENT")."<br>";
					$strSuccessMessage .= '<i>'.$execFile.'</i> - '.GetMessage("KDA_EE_CRON_EXEC_FILE_COMMENT")."<br>";
				...
				{
					$strSuccessMessage .= GetMessage("KDA_EE_CRON_SAVE_ONLY_FILE");
					if($_SERVER['SERVER_ADMIN']=='info@timeweb.ru')
					{
						$strSuccessMessage .= GetMessage("KDA_EE_CRON_SAVE_IN_PANEL_TIMEWEB")."<br><br><i>".str_replace(' '.$phpParams, '', mb_substr($cfg_subdata, mb_strlen($strTime))).'</i><br><br>';
					}
					else
					{
						$strSuccessMessage .= GetMessage("KDA_EE_CRON_SAVE_IN_PANEL")."<br><br><i>".mb_substr($cfg_subdata, mb_strlen($strTime)).'</i><br><br>';
					}
					$strSuccessMessage .= GetMessage("KDA_EE_CRON_WHERE_IS")."<br>";
					$strSuccessMessage .= '<i>'.$agent_php_path.'</i> - '.GetMessage("KDA_EE_CRON_PHP_PATH_COMMENT")."<br>";
					$strSuccessMessage .= '<i>'.$execFile.'</i> - '.GetMessage("KDA_EE_CRON_EXEC_FILE_COMMENT")."<br>";
	...
	if (strlen($strErrorMessage)<=0 && $_REQUEST["subaction"]=='delete')
	{
		if (true /*file_exists($docRoot."/bitrix/crontab/crontab.cfg")*/)
		{
			...
			{
				//$cfg_data = preg_replace("#^.*?".preg_quote($docRoot.KDA_EE_PATH2EXPORTS).$cronFrame." +".$PROFILE_ID." *>.*?$#im", "", $cfg_data);
				if(preg_match_all("#^.*?(\S+)".preg_quote(KDA_EE_PATH2EXPORTS).$cronFrame." +".$PROFILE_ID." *>.*?$#im", $cfg_data, $m))
				{
					foreach($m[0] as $k=>$v)
					{
...
$arRecords = array();
if(preg_match_all("#^\s*(([\*\-/,\d]+\s+){5})(\S+)\s+.*?(\S+)".preg_quote(KDA_EE_PATH2EXPORTS).$cronFrame." +(\d[\d,]*) *>.*?$#im", $cfg_data, $m))
{
	foreach($m[0] as $k=>$v)
	{
...
<form action="<?=$_SERVER['REQUEST_URI']?>" method="post" enctype="multipart/form-data" name="field_settings" class="kda-ee-cron-form">
		...
		<table width="100%">
			...
			<tr>
				<td colspan="2" style="text-align: center;">
					...
					<input type="submit" name="add" value="<? echo GetMessage("KDA_EE_CRON_SET"); ?>" onclick="return EProfile.SaveCron(this);" data-name-change="<? echo GetMessage("KDA_EE_CRON_CHANGE"); ?>" data-name-add="<? echo GetMessage("KDA_EE_CRON_SET"); ?>">
				</td>
			</tr>
		</table>
	</div>
		...
		<table width="100%" class="kda-ee-cron-records" cellspacing="8">
			...
			foreach($arRecords as $key=>$rec)
			{
				...
				echo '<tr>'.
					...
					'<td><a class="kda-ee-cron-record-edit" href="javascript:void(0)" onclick="return EProfile.EditCronRecord(this, \''.$key.'\');" title="'.htmlspecialcharsbx(GetMessage("KDA_EE_CRON_EDIT")).'" data-phppath="'.htmlspecialcharsbx($rec['phppath']).'" data-time="'.htmlspecialcharsbx($rec['time']).'" data-profiles="'.htmlspecialcharsbx(implode(',', $rec['pid'])).'"></a></td>'.
	...
	<?echo BeginNote();?>
		...
		<? if(!$cronWritable){echo '<br><br>'.GetMessage("KDA_EE_CRON_NOT_WRITABLE");} ?>
	<?echo EndNote();?>
</form>
