...
class CKDAFieldList {
			...
			if($arCatalog || $IBLOCK_ID===true)
			{
				...
				if(!$offers || Option::get('catalog', 'use_offer_marking_code_group')=='Y')
				{
					$arCatalogFields[] = array(
						...
						"name" => Loc::getMessage("KDA_IE_FI_UF_PRODUCT_GROUP"),
					);
				}
			}
		}
	...
	public static function GetReviewFields($sd = array())
	{
		$arReviewFields = array();
		if(Loader::includeModule('blog') && $sd['REVIEWS_BLOG'] > 0)
		{
			$arReviewFields = array(
				"REVIEW_AUTHOR_NAME" => Loc::getMessage("KDA_IE_FI_REVIEW_AUTHOR_NAME"),
				"REVIEW_POST_TEXT" => Loc::getMessage("KDA_IE_FI_REVIEW_POST_TEXT"),
				"REVIEW_DATE_CREATE" => Loc::getMessage("KDA_IE_FI_REVIEW_DATE_CREATE")
			);
			if(\CUserTypeEntity::GetList(array(), array('ENTITY_ID'=>'BLOG_COMMENT', 'FIELD_NAME'=>'UF_ASPRO_COM_RATING'))->Fetch())
			{
				$arReviewFields["REVIEW_UF_ASPRO_COM_RATING"] = Loc::getMessage("KDA_IE_FI_REVIEW_RATING");
			}
		}
		return (!empty($arReviewFields) ? $arReviewFields : false);
	}
	...
	public function GetFields($IBLOCK_ID, $offers = false)
	{
		...
		$key = $IBLOCK_ID.'_'.($offers ? 1 : 0);
		if(!$this->aFields[$key])
		{
			$this->aFieldsExData[$IBLOCK_ID] = array();
			$this->aFields[$key]['element'] = array(
			...
			foreach(self::GetIblockElementFields() as $k=>$ar)
			{
				...
				$this->aFields[$key]['element']['items'][$k] = $ar["name"];
			}
			
			if($arPropFields = self::GetIblockProperties($IBLOCK_ID))
			{
				$this->aFields[$key]['prop'] = array(
				...
				foreach($arPropFields as $ar)
				{
					...
					$this->aFields[$key]['prop']['items'][$ar["value"]] = $ar["name"];
					if($ar["wdesc"])
					{
						$this->aFields[$key]['prop']['items'][$ar["value"].'_DESCRIPTION'] = $ar["name"].' ('.Loc::getMessage("KDA_IE_PROP_DESCRIPTION").')';
					}
			...
			{
				$this->aFields[$key]['iprop_temp'] = array(
				...
				foreach($arIpropTempFields as $k=>$v)
				{
					...
					$this->aFields[$key]['iprop_temp']['items'][$k] = $v;
				}
			}
			
			if($arCatalogFields = self::GetCatalogFields($IBLOCK_ID, $offers))
			{
				$this->aFields[$key]['catalog'] = array(
				...
				foreach($arCatalogFields as $ar)
				{
					...
					$this->aFields[$key]['catalog']['items'][$ar["value"]] = $ar["name"];
				}
			}
			...
			{
				$this->aFields[$key]['catalog_discount'] = array(
				...
				{
					if($offers) $ar["value"] = 'OFFER_'.$ar["value"];
					$this->aFields[$key]['catalog_discount']['items'][$ar["value"]] = $ar["name"];
				}
			}
			
			if(/*!$offers &&*/ ($arCatalogSetFields = self::GetCatalogSetFields($IBLOCK_ID)))
			{
				$this->aFields[$key]['catalog_set'] = array(
				...
				{
					if($offers) $ar["value"] = 'OFFER_'.$ar["value"];
					$this->aFields[$key]['catalog_set']['items'][$ar["value"]] = $ar["name"];
				}
			}
			
			if(!$offers && ($arCatalogSetFields = self::GetCatalogSet2Fields($IBLOCK_ID)))
			{
				$this->aFields[$key]['catalog_set2'] = array(
					'title' => Loc::getMessage("KDA_IE_GROUP_CATALOG_SET2"),
					'items' => array()
				);
				foreach($arCatalogSetFields as $ar)
				{
					$this->aFields[$key]['catalog_set2']['items'][$ar["value"]] = $ar["name"];
				}
			}
			
			if($arRightsFields = self::GetElementRightsFields($IBLOCK_ID))
			{
				$this->aFields[$key]['element_rights'] = array(
				...
				{
					if($offers) $ar["value"] = 'OFFER_'.$ar["value"];
					$this->aFields[$key]['element_rights']['items'][$ar["value"]] = $ar["name"];
				}
			}
		}
	
		return $this->aFields[$key];
	}
	...
	public function GetFiltrableFields($IBLOCK_ID, $offers = false)
	{
		...
		if(!$this->aFiltrableFields[$IBLOCK_ID])
		{
				...
				$this->aFiltrableFields[$IBLOCK_ID]['section'] = array(
					...
					'items' => array(
						'IE_IBLOCK_SECTION' => array(
							"name" => Loc::getMessage("KDA_IE_ELEMENT_SECTION"),
							"type" => "section",
							"filtrable" => "Y"
						)
					)
			...
			$this->aFiltrableFields[$IBLOCK_ID]['element'] = array(
				//'title' => ($offers ? Loc::getMessage("KDA_IE_GROUP_OFFER") : Loc::getMessage("KDA_IE_GROUP_ELEMENT")),
				'title' => Loc::getMessage("KDA_IE_GROUP_ELEMENT"),
				'items' => array()
			);
			foreach(self::GetIblockElementFields() as $k=>$ar)
			{
			...
			if($arPropFields = self::GetIblockProperties($IBLOCK_ID))
			{
				$this->aFiltrableFields[$IBLOCK_ID]['prop'] = array(
					//'title' => ($offers ? Loc::getMessage("KDA_IE_GROUP_OFFER").' ('.Loc::getMessage("KDA_IE_GROUP_PROP").')' : Loc::getMessage("KDA_IE_GROUP_PROP")),
					'title' => Loc::getMessage("KDA_IE_GROUP_PROP"),
					'items' => array()
				);
				foreach($arPropFields as $ar)
				{
			...
			if($arCatalogFields = self::GetCatalogFields($IBLOCK_ID))
			{
				$this->aFiltrableFields[$IBLOCK_ID]['catalog'] = array(
					//'title' => ($offers ? Loc::getMessage("KDA_IE_GROUP_OFFER").' ('.Loc::getMessage("KDA_IE_GROUP_CATALOG").')' : Loc::getMessage("KDA_IE_GROUP_CATALOG")),
					'title' => Loc::getMessage("KDA_IE_GROUP_CATALOG"),
					'items' => array()
				);
				foreach($arCatalogFields as $ar)
				{
	...
	public function ShowSelectFields($IBLOCK_ID, $fname, $value="", $sd = array())
	{
		$arGroups = $this->GetFields($IBLOCK_ID);
		$arGroupsOffers = array();
		if($this->isSku)
		...
		if($arReviewFields = self::GetReviewFields($sd))
		{
			?><optgroup label="<?echo Loc::getMessage("KDA_IE_GROUP_REVIEWS")?>"><?
			foreach($arReviewFields as $k=>$v)
			{
	...
	public function ShowSelectPropertyList($IBLOCK_ID, $fname, $val=false, $prefix='', $onlyProps=false, $placeholder='')
	{
		$properties = $this->GetSelectPropertyList($IBLOCK_ID, $val, $prefix);
		if(strlen($placeholder)==0) $placeholder = Loc::getMessage('KDA_IE_PLACEHOLDER_CHOOSE');
		if(!$onlyProps) $catalogFields = $this->GetSelectCatalogList($IBLOCK_ID, $val, $prefix);
		else $catalogFields = '';
		?><select name="<?echo $fname;?>" class="kda-chosen-multi" multiple data-placeholder="<?echo $placeholder;?>">
		<?
	...
	public function ShowSelectSections($IBLOCK_ID, $fname, $value='', $mutiple=false, $disabled=false)
	{
		if(!$this->Sections)
		{
			$this->Sections = array();
		...
		?><select name="<?echo $fname;?>"<?if($mutiple){echo ' multiple';}?><?if($disabled){echo ' disabled';}?>><?
