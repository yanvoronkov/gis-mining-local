...
class CKDAImportUtils {
	...
	public static function GetOfferIblock($IBLOCK_ID, $retarray=false)
	{
		...
		if(!isset(self::$offerIblocks[$IBLOCK_ID]))
		{
			...
			{
				$arRels = \KdaIE\Utils::Unserialize(COption::GetOptionString(static::$moduleId, 'CATALOG_RELS'));
	...
	public static function MakeFileArray($path, $maxTime = 0)
	{
		...
		else
		{
						...
						))
			{
				...
				if(!$bExternalStorage)
				{
					...
					$download = false;
					if($ob->cdownload($path, $temp_path) && !in_array($ob->getStatus(), array(403, 404, 504)))
					{
						$download = true;
					}
						...
						|| (($content = ToLower(trim(file_get_contents($temp_path, false, null, 0, 65536)))) && strpos($content, '<?xml')===0))
					{
						...
						foreach($arNewPaths as $newPath)
						{
							...
							$download = false;
							if($ob->cdownload($newPath, $temp_path) && !in_array($ob->getStatus(), array(403, 404, 504)))
							{
								$download = true;
								$path = $newPath;
								break;
					...
					if(!$download && ($location = $ob->getHeaders()->get('location')) && $path!=$location)
					{
						...
						if($ob->cdownload($path, $temp_path))
						{
							$download = true;
						}
					}
					...
					while(!$download && in_array($ob->getStatus(), array(301, 302, 303, 307)) && ++$loop<=5)
					{
						...
						$download = $ob->cdownload($path, $temp_path);
					}
					...
					if(!$arFile && is_callable(array($ob, 'getLastError')) && ($error = $ob->getLastError()) && Loc::getMessage("KDA_IE_FILE_ERROR_".$error))
					{
						return array('ERROR_MESSAGE' => sprintf(Loc::getMessage("KDA_IE_FILE_ERROR_".$error), $ob->getLastErrorHost()));
					}
				}
				elseif($temp_path)
				{
			...
			elseif(preg_match('/ftp(es|s)?:\/\//', $path))
			{
		...
		{
			return array('ERROR_MESSAGE'=>GetMessage("KDA_IE_FILE_SIZE_0"));
		}
	...
	public static function CheckHtmlFile(&$arFile, $path)
	{
		...
		if((in_array($ext, array('htm', 'html')) || self::IsHtmlFile($arFile['tmp_name'], $arFile['tmp_name'], array('php'))) && class_exists('DOMDocument'))
		{
			...
			if(self::DetectUTF8($subcontent))
			{
				...
				if(mb_substr($content, 0, 3, 'latin1')!="\xEF\xBB\xBF")
				{
			...
			if(preg_match_all('#<\/?(thead|tbody)(\s+[^>]*>|>)#Uis', $content, $m))
			{
				$updateFile = true;
				foreach($m[0] as $m0)
				{
					$content = str_replace($m0, '', $content);
				}
			}
			
			if($updateFile)
			{
				file_put_contents($arFile['tmp_name'], $content);
			}
			/*/Bom UTF-8 and encoding*/
	...
	public static function GetShowFileBySettings($SETTINGS_DEFAULT)
	{
		...
		{
			$arParams = self::GetParamsFromEmailDataFile($SETTINGS_DEFAULT["EMAIL_DATA_FILE"]);
	...
	public static function GetParamsFromEmailDataFile($json)
	{
	...
	{
		$arAddFilter = \KdaIE\Utils::Unserialize(base64_decode($arAddFilter));
