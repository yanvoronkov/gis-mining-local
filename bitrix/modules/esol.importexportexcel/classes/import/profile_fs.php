...
class CKDAImportProfileFS extends CKDAImportProfileAll {
	...
	function __construct($suffix='')
	{
		...
		CheckDirPath($this->tmpcachedir);
		$this->archivesDir = $this->tmpdir.'_archives';
		CheckDirPath($this->archivesDir);
	...
	public function GetList($arFilter=array(), $groups = false)
	{
		...
		{
			$arProfiles = \KdaIE\Utils::Unserialize(file_get_contents($this->fileProfiles));
	...
	public function GetByID($ID)
	{
		...
		{
			$arProfile = \KdaIE\Utils::Unserialize(file_get_contents($fn));
	...
	public function Copy($ID)
	{
		...
		$arParams = \KdaIE\Utils::Unserialize(file_get_contents($fn));
