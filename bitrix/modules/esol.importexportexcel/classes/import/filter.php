...
class CKDAIEFilter {
	...
	public function GetListValues($field, $arParams=array())
	{
		...
		if($iblockId > 0) $offerIblockId = \CKDAImportUtils::GetOfferIblock($iblockId);
		if($iblockId > 0) $parentIblockId = \CKDAImportUtils::GetParentIblock($iblockId);
		...
		else
		{
			...
			elseif($field=='IE_IBLOCK_OFFER')
			{
				...
				if($cnt > $maxValuesCnt)
				{
					$arOfferIblock = \CKDAImportUtils::GetOfferIblock($IBLOCK_ID, true);
