...
class CKDAExportExcelHighload {
	...
	public function GetHighloadBlockValue($arProp, $val)
	{
		if(strlen($val) > 0 && CModule::IncludeModule('highloadblock') && $arProp['SETTINGS']['HLBLOCK_ID'] && $arProp['SETTINGS']['HLFIELD_ID'])
		{
			if(!isset($this->propVals[$arProp['ID']][$val]))
			{
				...
				$fieldName = 'ID';
				if($arField = CUserTypeEntity::GetList(array(), array('ID'=>$arProp['SETTINGS']['HLFIELD_ID']))->Fetch()) $fieldName = $arField['FIELD_NAME'];
				$arSelect = array_unique(array('ID', $fieldName));
				$dbRes2 = $entityDataClass::GetList(array('filter'=>array("ID"=>$val), 'select'=>$arSelect, 'limit'=>1));
				if($arr2 = $dbRes2->Fetch())
				{
					$this->propVals[$arProp['ID']][$val] = $arr2[$fieldName];
				}
				else
				{
					$this->propVals[$arProp['ID']][$val] = '';
	...
	public function GetExportData($listIndex, $limit=10, $page=1)
	{
		...
		if(!isset($this->filters[$hlblId][$listIndex]))
		{
			...
			{
				/*new filter*/
				if($this->params['HLFILTER'][$listIndex])
				{
					$eFilter = new CKDAEEFilter($hlblId);
					$eFilter->SetHlFilter($arFilter, $this->params['HLFILTER'][$listIndex], $this->fl);
				}
				/*/new filter*/
			}
				...
				foreach($this->fparams[$listIndex] as $fieldIndex=>$arSettings)
				{
					...
					if(isset($arSettings['CONVERSION']) && is_array($arSettings['CONVERSION']) && $field && isset($arElementData[$field]))
					{
						if(is_array($arElementData[$field]))
						{
							...
							{
								$arData[$k][$field.'_'.$fieldIndex][$k2] = $arElementData[$field.'_'.$fieldIndex][$k2] = $newVal = $this->ApplyConversions($val, $arSettings['CONVERSION'], $arElementData);
								if($newVal===false) $skipLine = true;
							}
						}
						else
						{
							$arData[$k][$field.'_'.$fieldIndex] = $arElementData[$field.'_'.$fieldIndex] = $newVal = $this->ApplyConversions($arElementData[$field], $arSettings['CONVERSION'], $arElementData);
							if($newVal===false) $skipLine = true;
						}
					}
