...
class CKDAEEFilter {
	...
	public function ShowSelectFilterFields($fl, $iblockId, $arFilter)
	{
		...
		?><select name="S_FIELD"><option value=""><?echo Loc::getMessage("KDA_EE_CHOOSE_FIELD");?></option><?if($this->type!='s'){?><option value="GROUP"><?echo Loc::getMessage("KDA_EE_FI_GROUP_COND");?></option><?}?><?
		...
		foreach($arFilter as $k=>$v)
		{
			if(isset($arFields[$v['FIELD']]) && isset($arFields[$v['FIELD']]['type']) && in_array($arFields[$v['FIELD']]['type'], array('section', 'list')))
			{
				...
				echo '<input type="hidden" name="FVALS_'.htmlspecialcharsbx($v['FIELD']).'" value="" data-val="'.htmlspecialcharsbx(\KdaIE\Utils::PhpToJSObject($arValues)).'">';
			}
		}
	}
	...
	public function GetListValues($field, $arParams=array())
	{
		...
		if($iblockId > 0) $offerIblockId = \CKDAExportUtils::GetOfferIblock($iblockId);
		if($iblockId > 0) $parentIblockId = \CKDAExportUtils::GetParentIblock($iblockId);
		...
		if($isHighload)
		{
			...
			{
				$arValues['1'] = Loc::getMessage("KDA_EE_YES");
			}
		...
		elseif($isSection || strpos($field, 'ISECT_')===0)
		{
			...
			{
				$arValues['text'] = Loc::getMessage("KDA_EE_TEXTTYPE");
				$arValues['html'] = Loc::getMessage("KDA_EE_HTMLTYPE");
			}
		...
		else
		{
			...
			{
				$arValues['Y'] = Loc::getMessage("KDA_EE_YES");
				$arValues['N'] = Loc::getMessage("KDA_EE_NO");
			}
			elseif($field=='ICAT_QUANTITY_TRACE')
			{
				$default = (\Bitrix\Main\Config\Option::get('catalog', 'default_quantity_trace', 'N')=='Y' ? Loc::getMessage("KDA_EE_YES") : Loc::getMessage("KDA_EE_NO"));
				$arValues['D'] = Loc::getMessage("KDA_EE_DEFAULT").' ('.$default.')';
				$arValues['Y'] = Loc::getMessage("KDA_EE_YES");
				$arValues['N'] = Loc::getMessage("KDA_EE_NO");
			}
			elseif($field=='ICAT_CAN_BUY_ZERO')
			{
				$default = (\Bitrix\Main\Config\Option::get('catalog', 'default_can_buy_zero', 'N')=='Y' ? Loc::getMessage("KDA_EE_YES") : Loc::getMessage("KDA_EE_NO"));
				$arValues['D'] = Loc::getMessage("KDA_EE_DEFAULT").' ('.$default.')';
				$arValues['Y'] = Loc::getMessage("KDA_EE_YES");
				$arValues['N'] = Loc::getMessage("KDA_EE_NO");
			}
			...
			{
				$default = (\Bitrix\Main\Config\Option::get('catalog', 'default_subscribe', 'N')=='Y' ? Loc::getMessage("KDA_EE_YES") : Loc::getMessage("KDA_EE_NO"));
				$arValues['D'] = Loc::getMessage("KDA_EE_DEFAULT").' ('.$default.')';
				$arValues['Y'] = Loc::getMessage("KDA_EE_YES");
				$arValues['N'] = Loc::getMessage("KDA_EE_NO");
			}
			...
			elseif($field=='IE_IBLOCK_OFFER')
			{
				...
				if($cnt > $maxValuesCnt)
				{
					$arOfferIblock = \CKDAExportUtils::GetOfferIblock($IBLOCK_ID, true);
