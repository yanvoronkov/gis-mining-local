    ...
    <ul class="pagination__list">
        ...
        if ($currentPage > 1): ?>
            <li class="pagination__item pagination__item--prev">
                <a href="<?=$sUrlPath?>?<?=$strNavQueryString?>PAGEN_<?=$navNum?>=1" class="pagination__link" aria-label="Первая страница">
        ...
        if ($currentPage > 1): ?>
            <li class="pagination__item pagination__item--prev">
                <a href="<?=$sUrlPath?>?<?=$strNavQueryString?>PAGEN_<?=$navNum?>=<?=($currentPage - 1)?>" class="pagination__link" aria-label="Предыдущая страница">
                    <svg width="8" height="13" viewBox="0 0 8 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M7.25 12.5L1.25 6.5L7.25 0.5" stroke="#6F7682" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </a>
            </li>
        <?php endif; ?>
        ...
        for ($i = $startPage; $i <= $endPage; $i++):
            ...
            <li class="pagination__item <?=$activeClass?>">
                <a href="<?=$sUrlPath?>?<?=$strNavQueryString?>PAGEN_<?=$navNum?>=<?=$i?>" class="pagination__link"><?=$i?></a>
            </li>
        <?php endfor;
        ...
        if ($endPage < $totalPages):
            ...
            <?php endif; ?>
            <li class="pagination__item">
                <a href="<?=$sUrlPath?>?<?=$strNavQueryString?>PAGEN_<?=$navNum?>=<?=$totalPages?>" class="pagination__link"><?=$totalPages?></a>
            </li>
        <?php endif; ?>
        ...
        if ($currentPage < $totalPages): ?>
            <li class="pagination__item pagination__item--next">
                <a href="<?=$sUrlPath?>?<?=$strNavQueryString?>PAGEN_<?=$navNum?>=<?=($currentPage + 1)?>" class="pagination__link" aria-label="Следующая страница">
                    <svg width="8" height="13" viewBox="0 0 8 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0.75 12.5L6.75 6.5L0.75 0.5" stroke="#131315" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </a>
            </li>
        <?php endif; ?>
        ...
        if ($currentPage < $totalPages): ?>
            <li class="pagination__item pagination__item--next">
                <a href="<?=$sUrlPath?>?<?=$strNavQueryString?>PAGEN_<?=$navNum?>=<?=$totalPages?>" class="pagination__link" aria-label="Последняя страница">
                    <svg width="14" height="11" viewBox="0 0 14 11" fill="none" xmlns="http://www.w3.org/2000/svg">
